...
public struct MLIndexCoverage: Sendable, Equatable {
    ...
    /// A tiny residual set must not make an otherwise useful search look stalled. These assets
    /// remain eligible for background retries; this affects presentation only.
    public var isSearchReadyWithMinorPendingWork: Bool {
        guard indexed > 0, pending > 0 else { return false }
        return pending <= max(3, total / 1_000)
    }
}
