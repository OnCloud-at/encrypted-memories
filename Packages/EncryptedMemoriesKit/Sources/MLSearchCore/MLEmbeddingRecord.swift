...
///
/// The composite key `(PhotoUID, MLModelDescriptor)` gives each model its own record epoch, makes
/// upserts idempotent for an asset, and lets the store reject mismatched vector dimensions explicitly.
///
...
public struct MLIndexBatchReport: Sendable {
    ...
    public init(
        total: Int = 0, indexed: Int = 0, skippedAlreadyIndexed: Int = 0, permanentFailure: Int = 0,
