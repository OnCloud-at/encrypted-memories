...
public struct MemoryConditions: Sendable, Equatable {
    ...
    /// events; on iOS/iPadOS it maps `didReceiveMemoryWarning` to `.critical` with purge-now semantics.
...
public final class MemoryPressureRegistration: @unchecked Sendable {
    private let lock = NSLock()
    private var cancel: (@MainActor () -> Void)?
    @MainActor init(cancel: @escaping @MainActor () -> Void) { self.cancel = cancel }
...
public final class MemoryPressureGovernor {
    ...
    public static let shared = MemoryPressureGovernor(runtimeState: .shared)

    public private(set) var conditions = MemoryConditions()
    public private(set) var tier: MemoryBudgetTier = .normal

    private final class Box {
        let handler: @MainActor (MemoryBudgetTier) -> Void
    ...
    public init(runtimeState: LibraryRuntimeState? = nil) {
    ...
    public func update(_ conditions: MemoryConditions) {
        ...
        runtimeState?.update {
            $0.memoryPressure = conditions.pressure
            $0.memoryBudgetTier = resolved
            $0.memoryHeadroom =
                switch conditions.pressure {
                case .normal: .healthy
                case .warning: .constrained
            ...
            $0.isLowPowerMode = conditions.lowPowerMode
        }
        ...
        // A callback can end its own registration or add another responder. Snapshot only the handlers
        // before dispatch so such mutations affect the next tier change, never this in-flight fan-out.
        let handlers = responders.map(\.handler)
        for handler in handlers { handler(resolved) }
    }
}
