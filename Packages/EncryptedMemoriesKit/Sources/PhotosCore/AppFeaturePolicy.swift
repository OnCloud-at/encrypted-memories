...
public struct AppFeaturePolicy: Sendable {
    ...
    public func availability(
        ...
        guard let requirement = requirements[feature],
            ...
            device.physicalMemoryBytes >= requirement.minimumPhysicalMemoryBytes
        else {
            return .unavailable
        }
    ...
    public static let production = AppFeaturePolicy(requirements: [
        // Smart Search ships with Apple Vision (no model download required); the optional
        // semantic model is dispatched to whatever compute device Core ML picks. The feature
        // must not be gated on a Neural Engine inventory snapshot.
        .smartSearch: AppFeatureRequirement(),
