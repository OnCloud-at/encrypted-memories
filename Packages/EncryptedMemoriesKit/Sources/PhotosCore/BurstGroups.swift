...
public enum BurstGroupResolver {
    /// Returns each member ID with its full group member IDs, sorted by capture time and then ID.
    /// Singleton candidates are intentionally omitted; the viewer only needs groups with multiple choices.
    ///
    /// Explicit `RelatedPhotos` edges are authoritative and may overlap. Burst-tagged rows without edges
    /// use adjacent capture times as a narrow fallback so unrelated bursts are not merged.
    public static func memberLookup(
        candidates: [BurstGroupCandidate],
        temporalClusterWindow: TimeInterval = 2.0
    ) -> [String: [String]] {
