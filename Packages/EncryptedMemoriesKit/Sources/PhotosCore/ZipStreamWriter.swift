...
///
/// Large entries use a ZIP data descriptor so CRC and size are accumulated while copying bounded chunks from a
/// file. Little-endian throughout, per the PKZIP APPNOTE.
public final class ZipStreamWriter {
    ...
    public func addFile(name: String, data: Data) throws {
        ...
        entries.append(Entry(name: nameBytes, crc: crc, size: size, localOffset: localOffset, flags: 0x0800))
    }

    /// Appends a file in bounded chunks. The data-descriptor form lets the CRC be computed in the same pass that
    /// writes the entry, so a multi-gigabyte video is neither materialized in RAM nor read twice.
    public func addFile(name: String, fileURL: URL, bufferSize: Int = 1_048_576) throws {
        let nameBytes = Array(name.utf8)
        guard nameBytes.count <= Int(UInt16.max) else { throw CocoaError(.fileWriteInvalidFileName) }
        let declaredSize = UInt64(try fileURL.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0)
        let sizeOver = declaredSize >= 0xFFFF_FFFF
        let localOffset = offset
        let flags: UInt16 = 0x0808  // UTF-8 filename + trailing data descriptor

        var header: [UInt8] = []
        appendU32(&header, 0x0403_4b50)
        appendU16(&header, sizeOver ? 45 : 20)
        appendU16(&header, flags)
        appendU16(&header, 0)
        appendU16(&header, dosTime)
        appendU16(&header, dosDate)
        appendU32(&header, 0)
        appendU32(&header, sizeOver ? 0xFFFF_FFFF : 0)
        appendU32(&header, sizeOver ? 0xFFFF_FFFF : 0)
        appendU16(&header, UInt16(nameBytes.count))
        let extra = sizeOver ? zip64Extra([declaredSize, declaredSize]) : []
        appendU16(&header, UInt16(extra.count))
        ...
        var size: UInt64 = 0
        while let chunk = try source.read(upToCount: max(4_096, bufferSize)), !chunk.isEmpty {
            try Task.checkCancellation()
            crc = Self.updateCRC32(crc, with: chunk)
            size += UInt64(chunk.count)
            try write(chunk)
        }
        guard size == declaredSize else { throw CocoaError(.fileReadCorruptFile) }
        crc ^= 0xFFFF_FFFF

        var descriptor: [UInt8] = []
        appendU32(&descriptor, 0x0807_4b50)
        appendU32(&descriptor, crc)
        if sizeOver {
            appendU64(&descriptor, size)
            appendU64(&descriptor, size)
        } else {
            appendU32(&descriptor, UInt32(size))
            appendU32(&descriptor, UInt32(size))
        }
        try write(descriptor)
        entries.append(Entry(name: nameBytes, crc: crc, size: size, localOffset: localOffset, flags: flags))
    }
    ...
    public func finish() throws {
        ...
        let needsZip64 =
            count >= 0xFFFF || cdSize >= 0xFFFF_FFFF || cdStart >= 0xFFFF_FFFF
        ...
        if needsZip64 {
            ...
            appendU16(&z, 45)
            appendU32(&z, 0)
            appendU32(&z, 0)  // this disk / disk with CD start
            appendU64(&z, count)
            appendU64(&z, count)
            appendU64(&z, cdSize)
            appendU64(&z, cdStart)
        ...
        appendU16(&e, 0)
        appendU16(&e, 0)  // this disk / disk with CD
    ...
    private static func updateCRC32(_ initial: UInt32, with data: Data) -> UInt32 {
        var crc = initial
