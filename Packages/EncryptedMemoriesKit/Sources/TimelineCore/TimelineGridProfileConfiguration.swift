...
package enum TimelineGridProfileConfigurationError: Error, CustomStringConvertible {
    ...
    case transitionDoesNotMatchLevelSemantics(
        profileID: String, level: Int, configured: GridTransitionKind, derived: GridTransitionKind)
    case cannotDeriveSemanticTransition(
        profileID: String, level: Int, from: GridLevelSemanticRole, to: GridLevelSemanticRole)
    ...
    package var description: String {
        ...
        case .emptyLevels(let profileID):
            return "grid profile \(profileID) contains no levels"
        case .invalidDefaultLevel(let profileID, let defaultLevel, let levelCount):
            return "grid profile \(profileID) default level \(defaultLevel) is outside 0..<\(levelCount)"
        case .invalidLevelID(let profileID, let expected, let actual):
            return "grid profile \(profileID) level id \(actual) must equal its index \(expected)"
        case .invalidNominalColumns(let profileID, let level, let value):
            return "grid profile \(profileID) level \(level) has invalid nominalColumns \(value)"
        case .invalidGap(let profileID, let level, let value):
            return "grid profile \(profileID) level \(level) has invalid gap \(value)"
        case .emptySupportedContentModes(let profileID, let level):
            return "grid profile \(profileID) level \(level) has no supported content modes"
        case .invalidContentMode(let profileID, let level, let value):
            return "grid profile \(profileID) level \(level) has invalid content mode \(value)"
        case .defaultContentModeNotSupported(let profileID, let level):
            return "grid profile \(profileID) level \(level) default content mode is not supported"
        case .transitionOnLastLevel(let profileID, let level):
            return "grid profile \(profileID) last level \(level) must not define transitionKindToNext"
        case .invalidTransitionKind(let profileID, let level, let value):
            return "grid profile \(profileID) level \(level) has invalid transition kind \(value)"
        case .transitionDoesNotMatchLevelSemantics(let profileID, let level, let configured, let derived):
            return
                "grid profile \(profileID) level \(level) transition \(configured.rawValue) does not match semantic transition \(derived.rawValue)"
        case .cannotDeriveSemanticTransition(let profileID, let level, let from, let to):
            return
                "grid profile \(profileID) level \(level) cannot derive transition from \(from.rawValue) to \(to.rawValue)"
        case .emptySelectionProfileID(let index):
            return "grid profile selection rule at index \(index) has an empty profile id"
        case .unknownSelectionProfileID(let id):
            return "grid profile selection rule references unknown profile id \(id)"
        case .invalidSelectionWidth(let profileID, let field, let value):
            return "grid profile selection rule for \(profileID) has invalid \(field) \(value)"
        case .invalidSelectionRange(let profileID, let min, let max):
            return "grid profile selection rule for \(profileID) has minLayoutWidth \(min) above maxLayoutWidth \(max)"
        case .invalidSelectionInputAffinity(let profileID, let value):
...
package struct TimelineGridProfileConfiguration: Equatable {
    ...
    package static func bundled(
        resourceName: String = "GridProfiles", bundle: Bundle = .module
    ) throws -> TimelineGridProfileConfiguration {
    ...
    private static func buildLevel(
        ...
        nextLevel: LevelDTO?,
        levelCount: Int
    ) throws -> GridLevelMetrics {
        guard level.id == index else {
            throw TimelineGridProfileConfigurationError.invalidLevelID(
                profileID: profileID, expected: index, actual: level.id)
        }
        ...
        let defaultMode = try defaultContentMode(
            profileID: profileID, index: index, level: level, supportedModes: supportedModes)
    ...
    private static func supportedContentModes(
        profileID: String, index: Int, level: LevelDTO
    ) throws -> Set<TileContentDisplayMode> {
        ...
        return try Set(
            level.supportedContentModes.map { raw -> TileContentDisplayMode in
                guard let mode = TileContentDisplayMode(rawValue: raw) else {
                    throw TimelineGridProfileConfigurationError.invalidContentMode(
                        profileID: profileID, level: index, value: raw)
                }
                return mode
            })
    }

    private static func defaultContentMode(
        profileID: String,
        index: Int,
        level: LevelDTO,
        supportedModes: Set<TileContentDisplayMode>
    ) throws -> TileContentDisplayMode {
        ...
        guard supportedModes.contains(defaultMode) else {
            throw TimelineGridProfileConfigurationError.defaultContentModeNotSupported(
                profileID: profileID, level: index)
        }
        return defaultMode
    }
    ...
    private static func transitionKind(
        ...
        nextRole: GridLevelSemanticRole?
    ) throws -> GridTransitionKind? {
        ...
        guard let nextRole,
            let derived = GridTransitionKind.semantic(from: currentRole, to: nextRole)
        else {
        ...
        guard let kind = GridTransitionKind(rawValue: raw) else {
            throw TimelineGridProfileConfigurationError.invalidTransitionKind(
                profileID: profileID, level: index, value: raw)
        }
    ...
    private static func buildSelectionRules(
        _ dtos: [SelectionRuleDTO],
        validProfileIDs: Set<String>
    ) throws -> [TimelineGridProfileSelectionRule] {
    ...
    private static func validateSelectionWidth(
        _ value: CGFloat?,
        profileID: String,
        field: String
    ) throws {
