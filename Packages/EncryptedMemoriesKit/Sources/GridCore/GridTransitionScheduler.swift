...
//
// Pure scheduling primitives shared by click and pinch transitions. A plan combines a C1 click profile,
// area-weighted frame allocation, center-out placement, and disjoint progress windows.
//
...
package enum GridTransitionScheduler {
    // Host-owned click progress with C1 trapezoidal velocity.
    ...
    package static func clickQSamples(
        durationSeconds d: Double, hz: Double, rampFraction: Double,
        phase: Double = 0
    ) -> [Double] {
        let n = Int(ceil(d * hz - 1e-9))
        return (0...n).map { k in
            clickQ(min(d, (Double(k) + phase) / hz), durationSeconds: d, rampFraction: rampFraction)
        }
    }

    // Area-weighted frame allocation. The focus component receives its minimum; other components start at one.
    ...
    // Remaining frames are distributed by visible area.
    package static func allocateFrames(
        components: [GridTransitionComponent], budget: Int,
        focusID: Int, focusMinFrames: Int
    ) -> [Int: Int] {
        ...
        var rem = budget - n
        while alloc[focusID, default: 1] < focusMinFrames, rem > 0 {
            alloc[focusID]! += 1
            rem -= 1
        }
        ...
        var i = 0
        while rem > 0 {
            alloc[allByArea[i % allByArea.count].id]! += 1
            rem -= 1
            i += 1
        }
        return alloc
    ...
    /// Left-to-right component order: largest area at the timeline centre (near peak geometry velocity),
    ...
    /// Builds variable, area-weighted progress windows with touching boundaries.
    package static func clickWindows(
        components: [GridTransitionComponent],
        tuning: GridTransitionTuning
    ) -> [Int: ClosedRange<Double>] {
        guard !components.isEmpty else { return [:] }
        let d = tuning.clickDurationSeconds
        let hz = tuning.planRefreshHz
        let r = tuning.clickRampFraction
        ...
        guard budget >= components.count else {
            return visibleSampledClickWindows(
                components: components, qSamples: qs, leadInFrames: li,
                leadOutFrames: lo, tuning: tuning)
        }
        let focus = components.max { $0.visibleAreaFraction < $1.visibleAreaFraction }!.id
        let alloc = allocateFrames(
    ...
    /// Overloaded click plans, such as wide 9-to-7 focus-row relayouts, can have too many relocation components for
    ...
    private static func visibleSampledClickWindows(
        components: [GridTransitionComponent],
        qSamples qs: [Double],
        leadInFrames li: Int,
    ...
    private static func windowsHaveInteriorSamples(
        _ windows: [Int: ClosedRange<Double>],
        qSamples qs: [Double],
        tuning: GridTransitionTuning
    ) -> Bool {
    ...
    /// Fixed-width pinch windows, ordered from the center out within a central band.
    package static func pinchWindows(
        components: [GridTransitionComponent],
        tuning: GridTransitionTuning
    ) -> [Int: ClosedRange<Double>] {
        guard !components.isEmpty else { return [:] }
