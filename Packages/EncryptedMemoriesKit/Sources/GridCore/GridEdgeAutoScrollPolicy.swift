...
/// toward the bottom (newest). The UIKit host only multiplies this value by the frame delta.
package enum GridEdgeAutoScrollPolicy {
    ...
    package static func velocity(
        ...
        // Top band: zero at the inner edge and -maxSpeed at or above the top.
        if touchY < band {
            let depth = (band - max(touchY, 0)) / band  // 0 at inner edge, 1 at/above the top
            return -maxSpeed * min(depth, 1)
        }

        // Bottom band: zero at the inner edge and +maxSpeed at or below the bottom.
