...
public struct UploadDestinationSheet: View {
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(L10n.string("action.cancel")) {
                        coordinator.cancelDestination()
                        dismiss()
                    }
                    .keyboardShortcut(.cancelAction)
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button(L10n.string("upload.confirm")) {
                        confirm()
                        dismiss()
                    }
    ...
    private var canConfirm: Bool {
        ...
        case .newAlbum: return coordinator.canCreateAlbum && !newAlbumName.trimmingCharacters(in: .whitespaces).isEmpty
        }
    }
    ...
    private var destination: UploadDestination? {
        let cover: UploadDestination.Cover =
            (useFirstAsCover && coordinator.canSetAlbumCover) ? .firstUploaded : .unchanged
        switch mode {
        case .library:
            return UploadDestination(target: .library)
        case .existing:
            guard let id = selectedAlbumID,
                let album = coordinator.albums.first(where: { $0.id == id })
            else {
                return nil
            }
        ...
        case .newAlbum:
            return UploadDestination(
                target: .newAlbum(name: newAlbumName.trimmingCharacters(in: .whitespaces)), cover: cover)
        }
    }
