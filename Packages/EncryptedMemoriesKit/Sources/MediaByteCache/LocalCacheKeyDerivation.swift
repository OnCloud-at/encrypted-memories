...
public enum LocalCacheKeyDerivation {
    public static func thumbnailPreviewCacheKey(accountUID: String, keyPassword: String) -> SymmetricKey {
        ...
        let salt = Data("EncryptedMemories.local-cache.v1.\(accountUID)".utf8)
...
/// One immutable security context for every encrypted local media derivative owned by an account session.
/// Platform shells choose cache roots and budgets; key derivation and multi-cache configuration happen once.
public struct LocalMediaCacheContext: Sendable {
    public let accountUID: String
    public let encryptionKey: SymmetricKey

    public init(accountUID: String, keyPassword: String) {
        self.accountUID = accountUID
        self.encryptionKey = LocalCacheKeyDerivation.thumbnailPreviewCacheKey(
            accountUID: accountUID,
            keyPassword: keyPassword
        )
    }

    public func configure(_ caches: ThumbnailCache...) {
        for cache in caches {
            cache.configure(accountUID: accountUID, key: encryptionKey)
        }
    }
}
