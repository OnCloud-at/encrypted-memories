...
//
// The toolbar button is a thin SwiftUI shell over this model so symbol resolution, vector fallback, and
// accessibility logic stay independent of the toolbar. Asset policy:
...
public enum AspectSquareToggleModel {
    ...
    /// The first candidate SF Symbol available on this OS, or nil when the caller should use `fallbackImage`.
    public static func resolvedSymbolName(for mode: TileContentDisplayMode) -> String? {
        for name in symbolCandidates[mode] ?? []
    ...
    /// A simple in-app vector glyph (CoreGraphics) used when no SF Symbol resolves: a square outline with
    /// a wide inner rectangle (aspect-fit) or an inner square (square-fill). The template image tints natively.
