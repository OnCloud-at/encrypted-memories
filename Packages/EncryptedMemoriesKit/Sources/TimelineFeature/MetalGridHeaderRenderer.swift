...
final class MetalGridHeaderRenderer {
    ...
    func reposition() {
        guard let coordinator, coordinator.showsMonthLabels, !markers.isEmpty else {
            hideAll()
            return
        }
        ...
        for (i, marker) in markers.enumerated() {
            let v = labels[i]
            guard let rect = coordinator.cellContentRect(forFlatIndex: marker.index) else {
                v.isHidden = true
                continue
            }
            let y = rect.minY - originY
            guard y >= -42, y <= vh else {
                v.isHidden = true
                continue
            }  // only labels whose row is on screen
