...
public enum UIKitDeviceMemoryClass: String, Sendable {
    /// ≤ ~4 GB devices - the constrained baseline. Foreground jetsam headroom is
...
public enum UIKitMediaCachePolicy {
    ...
    public static func memoryClass(
        physicalMemory: UInt64 = ProcessInfo.processInfo.physicalMemory
    ) -> UIKitDeviceMemoryClass {
        physicalMemory <= UInt64(4.5 * 1024 * 1024 * 1024) ? .constrained : .standard
    }

    /// Decoded thumbnail RAM budget (the dominant grid-hot cache).
    ///
    /// Memory-class curve: `constrained` (≤4 GB) devices use 5.5% with a hard 224 MiB ceiling.
    /// `standard` (≥6 GB) devices use the proportional 8% budget with a 96 MiB floor and 1 GiB ceiling.
    ///
