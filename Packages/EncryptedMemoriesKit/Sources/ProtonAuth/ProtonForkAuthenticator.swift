...
public enum ProtonAuthError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .apiError(let code, let message): L10n.string("error.auth_api \(code) \(message)")
...
public struct ProtonAPIConfig: Sendable {
    ...
    public static let externalDriveEncryptedMemories = ProtonAPIConfig()
    ...
    private func pollForkUntilReady(selector: String) async throws -> ForkStatus {
        ...
        while ContinuousClock.now < deadline {
            ...
            guard let uid = body.uid, let at = body.accessToken,
                let rt = body.refreshToken, let payload = body.payload
            else {
...
private struct ProtonErrorBody: Decodable {
    let code: Int
    let error: String?
