...
public actor CoreMLDualEncoder: MLAssetEmbedder, MLTextQueryEncoder {
    ...
    public func embed(uid: PhotoUID, descriptor: MLModelDescriptor) async -> MLEmbeddingOutcome {
        ...
        case .image(let source):
            ...
            } catch let error as CoreMLDualEncoderError {
                let failure = AppleSmartSearchRuntimeFailureTranslator.translate(error)
                if failure.isPermanent {
                    return .permanentFailure(reason: failure.debugDescription)
                }
    ...
    public func encode(text: String, descriptor: MLModelDescriptor) async throws -> ContiguousArray<Float32> {
        ...
        guard tokenized.inputIDs.count == schema.contextLength,
            ...
            tokenized.endTokenIndex < schema.contextLength
        else {
            throw CoreMLDualEncoderError.invalidStaticInputContract("tokenizer output")
        }
    ...
    private static func validate(
        ...
        guard let imageInput = imageDescription.inputDescriptionsByName[schema.imageInput],
            ...
            throw CoreMLDualEncoderError.invalidStaticInputContract(schema.imageInput)
        }
        if let side = schema.imagePixelSide {
            guard imageConstraint.pixelsWide == side, imageConstraint.pixelsHigh == side else {
                throw CoreMLDualEncoderError.invalidStaticInputContract(
        ...
        guard let tokenDescription = textDescription.inputDescriptionsByName[schema.tokenInput],
            ...
            tokenConstraint.shape.map(\.intValue).reduce(1, *) == schema.contextLength
        else {
            throw CoreMLDualEncoderError.invalidStaticInputContract(schema.tokenInput)
        }
        ...
        if let maskInputName = schema.endTokenMaskInput {
            guard let maskDescription = textDescription.inputDescriptionsByName[maskInputName],
                ...
                maskConstraint.shape.map(\.intValue).reduce(1, *) == schema.contextLength
            else {
                throw CoreMLDualEncoderError.invalidStaticInputContract(maskInputName)
            }
        } else {
            ...
            guard textDescription.inputDescriptionsByName.count == 1 else {
                throw CoreMLDualEncoderError.invalidStaticInputContract("unexpected extra text inputs")
            }
        }
    ...
    private static func validateOutput(
        ...
        guard let output = description.outputDescriptionsByName[schema.embeddingOutput],
            ...
            constraint.dataType == .float16 || constraint.dataType == .float32 || constraint.dataType == .double
        else {
            throw CoreMLDualEncoderError.invalidOutputSchema(schema.embeddingOutput)
        }
    }
