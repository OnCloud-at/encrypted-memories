...
final class AppFeaturePolicyTests: XCTestCase {
    func testSmartSearchRemainsAvailableWithoutNeuralEngine() {
        let device = AppDeviceCapabilities(available: [], physicalMemoryBytes: 8 << 30)
        XCTAssertEqual(
            AppFeaturePolicy.production.availability(of: .smartSearch, device: device, tier: .free),
            .available
        )
    }

    func testUnsupportedCapabilityHidesSpecializedFeatureBeforeTierEvaluation() {
        let device = AppDeviceCapabilities(available: [], physicalMemoryBytes: 8 << 30)
        XCTAssertEqual(
