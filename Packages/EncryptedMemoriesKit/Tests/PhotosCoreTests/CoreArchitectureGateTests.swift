...
final class CoreArchitectureGateTests: XCTestCase {
    ...
    func testDesignSystemKeepsSharedAndAppKitBoundariesSeparate() throws {
        ...
        for file in ["BrandLogo.swift", "ProtonComponents.swift", "ProtonColors.swift", "LoadingMark.swift"] {
        ...
        for file in coreFiles {
            ...
            if file.lastPathComponent == "TopFrostBar.swift" {
                for required in ["UIVisualEffectView", "NSVisualEffectView", "blendingMode = .withinWindow"]
                where !source.contains(required) {
                    violations.append(
                        "DesignSystemCore/TopFrostBar.swift: missing shared native frost bridge marker \(required)")
                }
                continue
            }
            // The mark itself is shared; only the genuinely-AppKit veil surface must stay out of core.
            for token in ["NSViewRepresentable", "NSVisualEffectView", "NSView", "FrostedGlassBackground"]
            where contains(token, in: code) {
                violations.append(
                    "DesignSystemCore/\(file.lastPathComponent): AppKit launch-veil symbol leaked into shared UI core (\(token))"
                )
            }
        }
        ...
        if FileManager.default.fileExists(atPath: exportsFile.path) {
            ...
            for required in ["@_exported import DesignSystemCore", "@_exported import DesignSystemAppKitAdapter"]
            where !exports.contains(required) {
    ...
    func testMetalGridRendererLivesBehindRenderingCoreGate() throws {
        ...
        if FileManager.default.fileExists(atPath: rendererFile.path) {
            ...
            let code = stripCommentsAndStringLiterals(from: source)
            for forbidden in ["MTKView", "MetalGridPalette", "PhotosCore", "MediaCache", "PhotoUID"]
            where contains(forbidden, in: code) {
                violations.append(
                    "MetalRenderingCore/MetalGridRenderer.swift: forbidden adapter/domain reference \(forbidden)")
            }
            if !source.contains("package final class MetalGridRenderer") {
                violations.append(
                    "MetalRenderingCore/MetalGridRenderer.swift: renderer must be package-visible to adapters")
            }
            if !source.contains("package func render(to target: MetalGridDrawableTarget") {
                violations.append(
                    "MetalRenderingCore/MetalGridRenderer.swift: renderer must expose drawable-target render entry")
            }
        }
        ...
        if FileManager.default.fileExists(atPath: adapterFile.path) {
            ...
            if !source.contains("import MetalKit") || !source.contains("init?(view: MTKView)") {
                violations.append(
                    "TimelineFeature/MetalGridRenderer+MTKView.swift: MTKView conversion belongs only in adapter")
            }
        }
    ...
    func testSmallPureGridHelpersStayInUniversalGridCore() throws {
        let helperNames = [
            ...
            "MetalGridGeometry.swift",
            "GridThumbnailOverlayPolicy.swift",
        ]
        ...
        for helperName in helperNames {
            ...
            if FileManager.default.fileExists(atPath: timelineFile.path) {
                violations.append(
                    "TimelineFeature/\(helperName): pure helper must stay out of the macOS adapter target")
            }
    ...
    func testThumbnailMetadataOverlaysStaySharedAcrossAppleGridHosts() throws {
        let gridPolicyFile =
            sourcesRoot
            .appendingPathComponent("GridCore")
            .appendingPathComponent("GridThumbnailOverlayPolicy.swift")
        let timelinePolicyFile =
            sourcesRoot
            .appendingPathComponent("TimelineCore")
            .appendingPathComponent("TimelineThumbnailOverlayPolicy.swift")
        let timelineResolverFile =
            sourcesRoot
            .appendingPathComponent("TimelineCore")
            .appendingPathComponent("TimelineThumbnailOverlayResolver.swift")
        let composerFile =
            sourcesRoot
            .appendingPathComponent("MetalGridComposeCore")
            .appendingPathComponent("MetalGridFrameComposer.swift")
        let macDataSourceFile =
            sourcesRoot
            .appendingPathComponent("TimelineFeature")
            .appendingPathComponent("MetalGridDataSource.swift")
        let iOSHostFile =
            sourcesRoot
            .appendingPathComponent("TimelineUIKitFeature")
            .appendingPathComponent("UIKitTimelineGridHost.swift")
        var violations: [String] = []

        for file in [
            gridPolicyFile, timelinePolicyFile, timelineResolverFile, composerFile, macDataSourceFile, iOSHostFile,
        ]
        where !FileManager.default.fileExists(atPath: file.path) {
            violations.append("\(file.lastPathComponent): missing shared thumbnail-overlay path")
        }

        if FileManager.default.fileExists(atPath: gridPolicyFile.path) {
            let imports = try importedModules(in: gridPolicyFile)
            if imports != ["CoreGraphics"] {
                violations.append(
                    "GridCore/GridThumbnailOverlayPolicy.swift: imports \(imports.sorted()) != [CoreGraphics]")
            }
            let source = try String(contentsOf: gridPolicyFile, encoding: .utf8)
            for symbol in [
                "package struct GridThumbnailOverlay",
                "package enum GridThumbnailOverlayPolicy",
                "minimumReadableSide",
                "displayedRect",
            ] where !source.contains(symbol) {
                violations.append("GridCore/GridThumbnailOverlayPolicy.swift: missing \(symbol)")
            }
            for forbidden in ["AppKit", "UIKit", "PhotoItem", "PhotoUID"] where source.contains(forbidden) {
                violations.append("GridCore/GridThumbnailOverlayPolicy.swift: universal policy contains \(forbidden)")
            }
        }

        if FileManager.default.fileExists(atPath: timelinePolicyFile.path) {
            let source = try String(contentsOf: timelinePolicyFile, encoding: .utf8)
            for symbol in [
                "TimelineThumbnailOverlayPolicy",
                "item.isVideo ? durationText",
                "item.tags.contains(.raw)",
            ] where !source.contains(symbol) {
                violations.append("TimelineCore/TimelineThumbnailOverlayPolicy.swift: missing \(symbol)")
            }
            for forbidden in ["AppKit", "UIKit", "SwiftUI"] where source.contains(forbidden) {
                violations.append("TimelineCore/TimelineThumbnailOverlayPolicy.swift: mapping contains \(forbidden)")
            }
        }

        if FileManager.default.fileExists(atPath: timelineResolverFile.path) {
            let source = try String(contentsOf: timelineResolverFile, encoding: .utf8)
            for symbol in [
                "TimelineThumbnailOverlayResolver",
                "TimelineThumbnailOverlayPolicy.overlay(for:",
                "PhotoMetadataProvider",
                "noteVisible",
            ] where !source.contains(symbol) {
                violations.append("TimelineCore/TimelineThumbnailOverlayResolver.swift: missing \(symbol)")
            }
            for forbidden in ["AppKit", "UIKit", "SwiftUI"] where source.contains(forbidden) {
                violations.append("TimelineCore/TimelineThumbnailOverlayResolver.swift: resolver contains \(forbidden)")
            }
        }
        ...
        if FileManager.default.fileExists(atPath: composerFile.path) {
            ...
            for symbol in [
                ...
                "metadataGlyphTextures",
                "buildTransitionGroups",
            ] where !source.contains(symbol) {
                violations.append("MetalGridComposeCore/MetalGridFrameComposer.swift: missing \(symbol)")
            }
            for forbidden in ["video.fill", "durationSeconds", "PhotoTag.raw"] where source.contains(forbidden) {
                violations.append(
                    "MetalGridComposeCore/MetalGridFrameComposer.swift: domain/platform detail leaked via \(forbidden)")
            }
        }

        if FileManager.default.fileExists(atPath: macDataSourceFile.path) {
            let source = try String(contentsOf: macDataSourceFile, encoding: .utf8)
            if !source.contains("TimelineThumbnailOverlayResolver") {
                violations.append(
                    "TimelineFeature/MetalGridDataSource.swift: macOS host must consume the shared resolver")
            }
        }
        if FileManager.default.fileExists(atPath: iOSHostFile.path) {
            let source = try String(contentsOf: iOSHostFile, encoding: .utf8)
            if !source.contains("TimelineThumbnailOverlayResolver") {
                violations.append(
                    "TimelineUIKitFeature/UIKitTimelineGridHost.swift: iOS host must consume the shared resolver")
            }
        }
        ...
        XCTAssertTrue(
            violations.isEmpty,
            """
            Thumbnail metadata overlay boundary regressed:
            \(violations.joined(separator: "\n"))

            Duration fetching/formatting, RAW classification, readability thresholds, and displayed-image
            positioning belong in universal Core. AppKit/UIKit may only feed visibility and rasterize the
            already-defined text glyph requests.
            """
        )
    }

    func testVisibleThumbnailPriorityStaysCoreOwnedAcrossAppleGridHosts() throws {
        let core = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("MediaFeedCore")
                .appendingPathComponent("ThumbnailFeedCore.swift"),
            encoding: .utf8
        )
        let macAdapter = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("MediaCacheAppKitAdapter")
                .appendingPathComponent("ThumbnailFeed.swift"),
            encoding: .utf8
        )
        let iOSAdapter = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("MediaCacheUIKitAdapter")
                .appendingPathComponent("UIKitThumbnailFeed.swift"),
            encoding: .utf8
        )
        ...
        let iOSHost = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("TimelineUIKitFeature")
                .appendingPathComponent("UIKitTimelineGridHostWarm.swift"),
            encoding: .utf8
        )

        XCTAssertTrue(core.contains("func warmVisibleDecoded("))
        XCTAssertTrue(core.contains("warmDecoded(requests, priority: .visibleNow, limit: limit)"))
        XCTAssertTrue(macAdapter.contains("core.warmVisibleDecoded(requests, limit: limit)"))
        XCTAssertTrue(macAdapter.contains("core.submitVisibleDiskDecodeDemand(requests)"))
        XCTAssertTrue(iOSAdapter.contains("core.warmVisibleDecoded(requests, limit: limit)"))
        XCTAssertTrue(macHost.contains("feed.submitVisibleDiskDecodeDemand(incoming)"))
        XCTAssertTrue(macHost.contains("feed.replaceVisiblePriorityDemand(missing)"))
        XCTAssertTrue(iOSHost.contains("thumbnailFeed.warmVisibleDecoded("))
        XCTAssertFalse(macHost.contains("priority: .zoomAnchorAndFocusRow"))
        XCTAssertFalse(core.contains("var results: [DecodedTile]"))
        guard let completionLoop = core.range(of: "for await tile in group"),
            let progressivePublish = core.range(
                of: "storeDecoded(image, for: tile.uid",
                range: completionLoop.upperBound..<core.endIndex
            ),
            let scheduleNext = core.range(
                of: "addNext()",
                range: completionLoop.upperBound..<core.endIndex
            )
        else {
            XCTFail("Visible decode completions must publish before the batch drains")
            return
        }
        XCTAssertLessThan(progressivePublish.lowerBound, scheduleNext.lowerBound)
    }

    func testThumbnailRevealStaysCoreOwnedAcrossAppleGridHosts() throws {
        let policy = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("GridCore")
                .appendingPathComponent("GridThumbnailRevealPolicy.swift"),
            encoding: .utf8
        )
        let cache = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("MetalGridTextureCore")
                .appendingPathComponent("MetalGridTextureCache.swift"),
            encoding: .utf8
        )
        ...
        let macHost = try String(
            contentsOf:
                sourcesRoot
                .appendingPathComponent("TimelineFeature")
                .appendingPathComponent("MetalGridCoordinator.swift"),
            encoding: .utf8
        )
        let iOSHost = try String(
            contentsOf:
        ...
        XCTAssertTrue(policy.contains("package enum GridThumbnailRevealPolicy"))
        XCTAssertTrue(cache.contains("thumbnailRevealStartedAt"))
        XCTAssertTrue(cache.contains("thumbnailRevealOpacity(for id:"))
        XCTAssertTrue(composer.contains("revealIDs: Set(visibleIDs)"))
        XCTAssertTrue(composer.contains("cache.thumbnailRevealOpacity(for: uid, now: now)"))
        XCTAssertTrue(macHost.contains("cache.hasActiveThumbnailReveal(in: visibleUIDs, now: now)"))
        XCTAssertTrue(iOSHost.contains("textureCache.hasActiveThumbnailReveal(in: ids.visible, now: now)"))
    }
    ...
    func testGridTextureBudgetShapeIsUniversalButDefaultsStayInAdapter() throws {
        ...
        // per-frame upload copy cost.
        for symbol in [
            "GridTextureBudget", "maxUploadsPerFrame", "maxUploadBytesPerFrame", "maxUploadMillisecondsPerFrame",
            "maxCachedTextures", "maxResidentBytes", "overscanFraction",
        ] {
            if !budgetCode.contains(symbol) {
                violations.append("GridCore/GridTextureBudget.swift: missing \(symbol)")
            }
        }
        for forbidden in ["PhotosCore", "PhotoUID", "MTLTexture", "MetalGridBudget", "static let `default`"]
        where budgetCode.contains(forbidden) {
        ...
        if timelineImports.contains("PhotosCore") {
            violations.append(
                "TimelineFeature/MetalGridTypes.swift: budget/stats value types must not import PhotosCore")
        }
        ...
        if timelineSource.contains("struct MetalGridBudget") {
            violations.append(
                "TimelineFeature/MetalGridTypes.swift: MetalGridBudget must stay a typealias to GridTextureBudget")
        }
        ...
        for forbidden in ["static let `default`", "GridTextureBudget(maxUploadsPerFrame:", "maxCachedTextures: 4096"]
        where timelineSource.contains(forbidden) {
            violations.append(
                "TimelineFeature/MetalGridTypes.swift: platform texture budget default must stay in an adapter, found \(forbidden)"
            )
        }
        ...
        if FileManager.default.fileExists(atPath: appKitPolicyFile.path) {
            ...
            ] where !containsIgnoringWhitespace(symbol, in: appKitSource) {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridTexturePolicy.swift: missing adapter-owned macOS default \(symbol)"
                )
            }
        } else {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTexturePolicy.swift: missing adapter-owned macOS default")
        }
    ...
    func testMetalGridTextureCacheUsesInjectedGlyphRasterizer() throws {
        ...
        for file in [cacheFile, glyphContractFile, appKitRasterizerFile, appKitFactoryFile]
        where !FileManager.default.fileExists(atPath: file.path) {
            violations.append(
                "\(file.deletingLastPathComponent().lastPathComponent)/\(file.lastPathComponent): missing glyph/cache boundary file"
            )
        }
        ...
        if FileManager.default.fileExists(atPath: cacheFile.path) {
            ...
            if imports.contains("AppKit") {
                violations.append(
                    "MetalGridTextureCore/MetalGridTextureCache.swift: texture cache must not import AppKit")
            }
            let source = try String(contentsOf: cacheFile, encoding: .utf8)
            let code = stripCommentsAndStringLiterals(from: source)
            for forbidden in ["NSImage", "NSColor", "NSFont", "lockFocus", "systemSymbolName", "renderGlyph"]
            where contains(forbidden, in: code) {
                violations.append(
                    "MetalGridTextureCore/MetalGridTextureCache.swift: glyph rasterization leaked into cache via \(forbidden)"
                )
            }
            if !source.contains("glyphRasterizer: any MetalGridGlyphRasterizing") {
                violations.append(
                    "MetalGridTextureCore/MetalGridTextureCache.swift: missing injected glyph rasterizer dependency")
            }
            if !source.contains("glyphRasterizer.image(for: request)") {
                violations.append(
                    "MetalGridTextureCore/MetalGridTextureCache.swift: glyph image must come from injected rasterizer")
            }
        }
        ...
        if FileManager.default.fileExists(atPath: glyphContractFile.path) {
            ...
            if imports != ["CoreGraphics"] {
                violations.append(
                    "MetalGridTextureCore/MetalGridGlyphRasterizer.swift: imports \(imports.sorted()) != [CoreGraphics]"
                )
            }
            let source = try String(contentsOf: glyphContractFile, encoding: .utf8)
            for symbol in ["MetalGridGlyphRequest", "MetalGridGlyphColor", "MetalGridGlyphRasterizing"]
            where !source.contains(symbol) {
                violations.append("MetalGridTextureCore/MetalGridGlyphRasterizer.swift: missing \(symbol)")
            }
        }
        ...
        if FileManager.default.fileExists(atPath: appKitRasterizerFile.path) {
            ...
            if !source.contains("import AppKit") || !source.contains("import MetalGridTextureCore")
                || !source.contains("package final class AppKitMetalGridGlyphRasterizer")
            {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: AppKit implementation must own native glyph rasterization"
                )
            }
            if !source.contains("NSImage(systemSymbolName:") || !source.contains("NSImage.SymbolConfiguration") {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: expected AppKit SF Symbol path"
                )
            }
            for symbol in ["case .text", "NSAttributedString", "NSFont.monospacedSystemFont"]
            where !source.contains(symbol) {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: missing native text path \(symbol)"
                )
            }
        }
        ...
        if FileManager.default.fileExists(atPath: appKitFactoryFile.path) {
            ...
            if !source.contains("glyphRasterizer: any MetalGridGlyphRasterizing = AppKitMetalGridGlyphRasterizer()") {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: AppKit cache factory must inject the default AppKit rasterizer"
                )
            }
            if !source.contains("MetalGridTextureCache(") || !source.contains("glyphRasterizer: glyphRasterizer") {
                violations.append(
                    "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: factory must pass the injected rasterizer into the shared cache"
                )
            }
        }
        ...
        if FileManager.default.fileExists(atPath: coordinatorFile.path) {
            ...
            if !source.contains("import MetalGridTextureCore") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: macOS adapter must import the texture core explicitly")
            }
            if !source.contains("import MetalGridTextureAppKitAdapter") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: macOS adapter must import the AppKit texture adapter explicitly"
                )
            }
            if !source.contains("AppKitMetalGridTextureCacheFactory.makeCache") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: macOS adapter must assemble the texture cache through the AppKit factory"
                )
            }
            if source.contains("glyphRasterizer: AppKitMetalGridGlyphRasterizer()") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: direct glyph injection belongs in the AppKit cache factory"
                )
            }
            if !source.contains("MetalGridGlyphColor(.controlAccentColor)") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: AppKit colors must convert at adapter edge")
            }
        }
    ...
    func testAppKitGlyphRasterizerStaysInPlatformAdapter() throws {
        ...
        if let targetLine = manifestLine(forTarget: "MetalGridTextureAppKitAdapter", in: manifest) {
            ...
            if dependencies != ["GridCore", "MetalGridTextureCore"] {
                violations.append(
                    "MetalGridTextureAppKitAdapter: dependencies \(dependencies.sorted()) != [GridCore, MetalGridTextureCore]"
                )
            }
        } else {
        ...
        if imports != expectedImports {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
            )
        }
        for forbidden in ["UIKit", "SwiftUI", "MetalKit", "PhotosCore", "MediaCache", "TimelineFeature"]
        where imports.contains(forbidden) {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: must not import \(forbidden)")
        }
        ...
        ] where !containsIgnoringWhitespace(symbol, in: source) {
            violations.append("MetalGridTextureAppKitAdapter/AppKitMetalGridGlyphRasterizer.swift: missing \(symbol)")
        }

        if FileManager.default.fileExists(
            atPath: timelineRoot.appendingPathComponent("AppKitMetalGridGlyphRasterizer.swift").path)
        {
            violations.append(
                "TimelineFeature/AppKitMetalGridGlyphRasterizer.swift: AppKit glyph rasterization belongs in MetalGridTextureAppKitAdapter"
            )
        }
        ...
        if FileManager.default.fileExists(atPath: coordinatorFile.path) {
            ...
            if !coordinator.contains("AppKitMetalGridTextureCacheFactory.makeCache") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: must use the AppKit texture cache factory")
            }
        }
    ...
    func testAppKitTextureBudgetsStayAdapterOwned() throws {
        ...
        guard FileManager.default.fileExists(atPath: policyFile.path) else {
            XCTFail(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTexturePolicy.swift: missing AppKit texture budget policy"
            )
            return
        }
        ...
        if imports != ["CoreGraphics", "GridCore"] {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTexturePolicy.swift: imports \(imports.sorted()) != [CoreGraphics, GridCore]"
            )
        }
        ...
        for forbidden in [
            "ProcessInfo.processInfo.physicalMemory", "activeProcessorCount", "UIDevice", "userInterfaceIdiom",
            "PhotoUID", "PhotoItem", "TimelineFeature",
        ] where contains(forbidden, in: code) {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTexturePolicy.swift: platform policy must not bind hardware probes or photo-domain types, found \(forbidden)"
            )
        }
        ...
        where timelineSource.contains(forbidden) {
            violations.append(
                "TimelineFeature/MetalGridTypes.swift: AppKit texture policy must stay adapter-owned, found \(forbidden)"
            )
        }
    ...
    func testAppKitTextureCacheFactoryUsesSharedTextureCore() throws {
        ...
        guard FileManager.default.fileExists(atPath: factoryFile.path) else {
            XCTFail(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: missing AppKit cache factory")
            return
        }
        ...
        if imports != expectedImports {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
            )
        }
        for forbidden in ["UIKit", "SwiftUI", "MetalKit", "PhotosCore", "MediaCache", "TimelineFeature"]
        where imports.contains(forbidden) {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: must not import \(forbidden)")
        }
        ...
        for symbol in [
            ...
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: missing \(symbol)")
        }
        for forbidden in [
            "PhotoUID", "PhotoItem", "MetalGridBudget.default", "GridTextureBudget(", "maxUploadsPerFrame:",
            "maxCachedTextures:",
        ] where contains(forbidden, in: code) {
            violations.append(
                "MetalGridTextureAppKitAdapter/AppKitMetalGridTextureCacheFactory.swift: factory must use injected AppKit policy and generic IDs, found \(forbidden)"
            )
        }
        ...
        if FileManager.default.fileExists(atPath: coordinatorFile.path) {
            ...
            if !coordinator.contains("AppKitMetalGridTexturePolicies.policy(budget: budget)") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: macOS coordinator must wrap its budget in an AppKit texture policy"
                )
            }
            if !coordinator.contains("AppKitMetalGridTextureCacheFactory.makeCache") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: macOS coordinator must use the AppKit texture cache factory"
                )
            }
            if coordinator.contains("MetalGridTextureCache<PhotoUID>(") {
                violations.append(
                    "TimelineFeature/MetalGridCoordinator.swift: direct cache construction should stay behind the AppKit factory"
                )
            }
        }
    ...
    func testUIKitTextureBudgetsStayAdapterOwnedAndConservative() throws {
        ...
        guard FileManager.default.fileExists(atPath: policyFile.path) else {
            XCTFail(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTexturePolicy.swift: missing UIKit texture budget policy")
            return
        }
        ...
        if imports != ["CoreGraphics", "GridCore"] {
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTexturePolicy.swift: imports \(imports.sorted()) != [CoreGraphics, GridCore]"
            )
        }
        ...
        for forbidden in [
            "ProcessInfo.processInfo.physicalMemory", "activeProcessorCount", "UIDevice", "userInterfaceIdiom",
            "MetalGridBudget.default",
        ] where code.contains(forbidden) {
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTexturePolicy.swift: budget policy must be viewport/capability injected, found \(forbidden)"
            )
        }
        for macValue in [
            ...
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTexturePolicy.swift: UIKit policy must not copy macOS default \(macValue)"
            )
        }
        ...
        if timelineSource.contains("static let `default` = GridTextureBudget(") {
            violations.append(
                "TimelineFeature/MetalGridTypes.swift: macOS default budget belongs in MetalGridTextureAppKitAdapter")
        }
    ...
    func testUIKitTextureCacheFactoryUsesSharedTextureCore() throws {
        ...
        if imports != expectedImports {
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTextureCacheFactory.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
            )
        }
        for forbidden in ["AppKit", "SwiftUI", "MetalKit", "PhotosCore", "MediaCache", "TimelineFeature"]
        where imports.contains(forbidden) {
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTextureCacheFactory.swift: must not import \(forbidden)")
        }
        ...
        for forbidden in [
            ...
            violations.append(
                "MetalGridTextureUIKitAdapter/UIKitMetalGridTextureCacheFactory.swift: factory must use injected UIKit policy and generic IDs, found \(forbidden)"
            )
        }
    ...
    func testUIKitMediaCacheAdapterStaysThinAndCoreBacked() throws {
        ...
        if let targetLine = manifestLine(forTarget: "MediaCacheUIKitAdapter", in: manifest) {
            ...
            let expected: Set<String> = [
                "PhotosCore", "LibraryRuntimeAppleAdapter", "MediaByteCache", "MediaDecodingCore",
                "MediaFeedCore", "MediaCacheCore",
            ]
            if dependencies != expected {
                violations.append(
                    "MediaCacheUIKitAdapter: dependencies \(dependencies.sorted()) != \(expected.sorted())")
            }
        } else {
        ...
        if FileManager.default.fileExists(atPath: feedFile.path) {
            ...
            if imports != expectedImports {
                violations.append(
                    "MediaCacheUIKitAdapter/UIKitThumbnailFeed.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
            ...
            for forbidden in [
                "AppKit", "SwiftUI", "Metal", "MetalKit", "TimelineFeature", "PhotoViewerFeature", "MapFeature",
                "ProtonDriveSDK", "NSImage",
            ] where contains(forbidden, in: code) {
                violations.append(
                    "MediaCacheUIKitAdapter/UIKitThumbnailFeed.swift: forbidden adapter dependency/reference \(forbidden)"
                )
            }
        }
        ...
        if FileManager.default.fileExists(atPath: policyFile.path) {
            ...
            if imports != ["Foundation", "MediaByteCache", "os"] {
                violations.append(
                    "MediaCacheUIKitAdapter/UIKitMediaCachePolicy.swift: imports \(imports.sorted()) != [Foundation, MediaByteCache, os]"
                )
            }
            ...
            for macValue in [
                ...
                violations.append(
                    "MediaCacheUIKitAdapter/UIKitMediaCachePolicy.swift: UIKit policy must not copy macOS default \(macValue)"
                )
            }
        }
        ...
        if FileManager.default.fileExists(atPath: decoderFile.path) {
            ...
            if imports != ["MediaDecodingCore", "UIKit"] {
                violations.append(
                    "MediaCacheUIKitAdapter/UIKitThumbnailImageDecoder.swift: imports \(imports.sorted()) != [MediaDecodingCore, UIKit]"
                )
            }

            let source = try String(contentsOf: decoderFile, encoding: .utf8)
            for symbol in ["UIImage(cgImage: decoded.image", "decodedCost(_ image: UIImage)", "image.cgImage"]
            where !source.contains(symbol) {
    ...
    func testMapUIKitAdapterStaysPlatformOnlyAndLocationCoreBacked() throws {
        ...
        let loaderFile = sourcesRoot.appendingPathComponent("MapCore/PhotoMapAnnotationLoader.swift")
        let viewportFile = sourcesRoot.appendingPathComponent("MediaLocationCore/PhotoLocationViewport.swift")
        var violations: [String] = []
        ...
        if let targetLine = manifestLine(forTarget: "MapUIKitAdapter", in: manifest) {
            ...
            if dependencies != ["PhotosCore", "MediaLocationCore", "MapCore"] {
                violations.append(
                    "MapUIKitAdapter: dependencies \(dependencies.sorted()) != [MapCore, MediaLocationCore, PhotosCore]"
                )
            }
        } else {
        ...
        if FileManager.default.fileExists(atPath: hostFile.path) {
            ...
            if imports != expectedImports {
                violations.append(
                    "MapUIKitAdapter/UIKitLibraryMapHostView.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
        ...
        if FileManager.default.fileExists(atPath: viewsFile.path) {
            ...
            let expectedImports: Set<String> = ["MapKit", "MapCore", "PhotosCore", "UIKit"]
            if imports != expectedImports {
                violations.append(
                    "MapUIKitAdapter/UIKitPhotoAnnotationViews.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
            ...
            let code = stripCommentsAndStringLiterals(from: source)
            let clusteringAssignments = code.split(separator: "\n").filter { line in
                line.contains("clusteringIdentifier") && line.contains("=") && !line.contains("==")
                    && !line.contains("!=")
            }
            if clusteringAssignments.count != 1 || !clusteringAssignments[0].contains("clusteringIdentifier = nil") {
                violations.append(
                    "MapUIKitAdapter/UIKitPhotoAnnotationViews.swift: clusteringIdentifier must be assigned nil exactly once"
                )
            }

            let annotationViewSubclassCount = code.components(separatedBy: ": MKAnnotationView").count - 1
            if annotationViewSubclassCount != 1 {
                violations.append(
                    "MapUIKitAdapter/UIKitPhotoAnnotationViews.swift: expected one final annotation view, found \(annotationViewSubclassCount)"
                )
            }
        }

        if FileManager.default.fileExists(atPath: loaderFile.path) {
            let source = try String(contentsOf: loaderFile, encoding: .utf8)
            for symbol in [
                "PhotoLocationAggregationPlan",
                "policy.aggregationPlan(for: viewport, viewportSize: viewportSize)",
                "policy.aggregatedCoordinates(from: coordinates, using: plan)",
                "Task.detached(priority: .userInitiated)",
            ] where !source.contains(symbol) {
                violations.append(
                    "MapCore/PhotoMapAnnotationLoader.swift: missing shared aggregation invariant \(symbol)")
            }
        } else {
            violations.append("MapCore/PhotoMapAnnotationLoader.swift: missing shared map loader")
        }

        if FileManager.default.fileExists(atPath: viewportFile.path) {
            let source = try String(contentsOf: viewportFile, encoding: .utf8)
            for symbol in [
                ...
                violations.append(
                    "MediaLocationCore/PhotoLocationViewport.swift: missing shared aggregation contract \(symbol)")
            }
        } else {
            violations.append("MediaLocationCore/PhotoLocationViewport.swift: missing shared viewport policy")
        }

        for file in try swiftFiles(in: adapterRoot) {
            let imports = try importedModules(in: file)
            for forbidden in [
                "AppKit", "SwiftUI", "DesignSystem", "MediaCache", "TimelineFeature", "PhotoViewerFeature",
                "MapFeature", "ProtonDriveSDK",
            ] where imports.contains(forbidden) {
            ...
            let code = stripCommentsAndStringLiterals(from: source)
            for forbidden in [
                "NSImage", "NSView", "NSScrollView", "UIDevice", "userInterfaceIdiom", "UIScreen.main",
                "ThumbnailFeed", "PhotoViewerModel", "MKClusterAnnotation",
                "MKMapViewDefaultClusterAnnotationViewReuseIdentifier", "displayPriority = .defaultLow",
            ] where contains(forbidden, in: code) {
                violations.append(
                    "MapUIKitAdapter/\(file.lastPathComponent): forbidden macOS/feature/hardware reference \(forbidden)"
                )
            }
        }
        ...
        XCTAssertTrue(
            ...
            MapCore owns the shared annotation loader, while MediaLocationCore owns the stable aggregation plan.
            Native MapKit re-clustering or decluttering must not hide Core cells or make their counts lossy.
            """
        )
    }
    ...
    func testTimelineUIKitAdapterUsesViewportDrivenCoreProfileResolver() throws {
        ...
        if let targetLine = manifestLine(forTarget: "TimelineUIKitAdapter", in: manifest) {
            ...
            if dependencies != ["GridCore", "MetalRenderingCore", "TimelineCore"] {
                violations.append(
                    "TimelineUIKitAdapter: dependencies \(dependencies.sorted()) != [GridCore, MetalRenderingCore, TimelineCore]"
                )
            }
        } else {
        ...
        if imports != expectedImports {
            violations.append(
                "TimelineUIKitAdapter/UIKitTimelineGridProfileAdapter.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
            )
        }
        for forbidden in ["AppKit", "SwiftUI", "Metal", "MetalKit", "PhotosCore", "MediaCache", "TimelineFeature"]
        where imports.contains(forbidden) {
            violations.append(
                "TimelineUIKitAdapter/UIKitTimelineGridProfileAdapter.swift: must not import \(forbidden)")
        }
        ...
        for forbidden in [
            ...
            violations.append(
                "TimelineUIKitAdapter/UIKitTimelineGridProfileAdapter.swift: viewport adapter leaked forbidden policy/domain type \(forbidden)"
            )
        }
    ...
    func testMetal3RuntimeCapabilityOwnsSharedAppleHardwareFloor() throws {
        let capabilityFile =
            sourcesRoot
            .appendingPathComponent("MetalRenderingCore/Metal3RuntimeCapability.swift")
        let source = try String(contentsOf: capabilityFile, encoding: .utf8)

        XCTAssertEqual(try importedModules(in: capabilityFile), ["Metal"])
        for symbol in [
            ...
            XCTAssertTrue(source.contains(symbol), "shared Metal 3 runtime gate missing \(symbol)")
        }
    }
    ...
    func testTimelineUIKitAdapterOwnsIOSMetalSurfaceOnly() throws {
        ...
        for file in [hostFile, drawableFile, displayLinkFile, capabilityFile]
        where !FileManager.default.fileExists(atPath: file.path) {
        ...
        if FileManager.default.fileExists(atPath: hostFile.path) {
            ...
            if imports != expectedImports {
                violations.append(
                    "TimelineUIKitAdapter/UIKitTimelineMetalHostView.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
            let source = try String(contentsOf: hostFile, encoding: .utf8)
            for symbol in [
        ...
        if FileManager.default.fileExists(atPath: drawableFile.path) {
            ...
            if imports != expectedImports {
                violations.append(
                    "TimelineUIKitAdapter/UIKitTimelineMetalDrawableTarget.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
        ...
        if FileManager.default.fileExists(atPath: displayLinkFile.path) {
            ...
            if imports != expectedImports {
                violations.append(
                    "TimelineUIKitAdapter/UIKitTimelineDisplayLinkDriver.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
        ...
        if FileManager.default.fileExists(atPath: capabilityFile.path) {
            ...
            let expectedImports: Set<String> = ["Metal", "MetalRenderingCore"]
            if imports != expectedImports {
                violations.append(
                    "TimelineUIKitAdapter/UIKitTimelineMetalCapability.swift: imports \(imports.sorted()) != \(expectedImports.sorted())"
                )
            }
            let source = try String(contentsOf: capabilityFile, encoding: .utf8)
            for symbol in [
                ...
                "true",
                "Metal3RuntimeCapability.supports(device: device)",
            ] where !source.contains(symbol) {
        ...
        for file in try swiftFiles(in: adapterRoot) {
            let imports = try importedModules(in: file)
            for forbidden in [
                "AppKit", "SwiftUI", "MetalKit", "PhotosCore", "MediaCache", "TimelineFeature", "ProtonDriveSDK",
            ] where imports.contains(forbidden) {
            ...
            let code = stripCommentsAndStringLiterals(from: source)
            for forbidden in [
                "MTKView", "NSView", "NSScrollView", "NSImage", "PhotoUID", "PhotoItem", "ThumbnailFeed", "UIDevice",
                "userInterfaceIdiom", "UIScreen.main",
            ] where contains(forbidden, in: code) {
                violations.append(
                    "TimelineUIKitAdapter/\(file.lastPathComponent): forbidden macOS/domain/hardware reference \(forbidden)"
                )
            }
        }
    ...
    func testTimelineUIKitFeatureOwnsIOSPhotoGridAssembly() throws {
        ...
        for file in featureFiles {
            ...
            for forbidden in [
                ...
                violations.append(
                    "TimelineUIKitFeature/\(file.lastPathComponent): forbidden macOS/SDK/duplicated-sequence reference \(forbidden)"
                )
            }
        }
        ...
        XCTAssertTrue(
            ...
            The UIKit timeline feature may assemble shared GridCore, MediaFeedCore wake subscriptions,
            MetalRenderingCore, texture cache, and the UIKit thumbnail feed for iOS/iPadOS. macOS
            TimelineFeature/AppKit hosts, SDK glue, and desktop cache policy must stay outside this target.
            """
        )
    }
    ...
    func testMetalGridTextureCacheStaysItemIDGeneric() throws {
        ...
        for forbidden in ["PhotoUID", "PhotoItem", "PhotosCore"] where contains(forbidden, in: code) {
            violations.append(
                "MetalGridTextureCore/MetalGridTextureCache.swift: cache must stay item-ID generic; found \(forbidden)")
        }
        if !source.contains("package final class MetalGridTextureCache<ID: Hashable & Sendable>") {
            violations.append(
                "MetalGridTextureCore/MetalGridTextureCache.swift: cache must be package-visible and generic over a sendable hashable item ID"
            )
        }
        if !source.contains("GridTextureResidencyPolicy<ID>") || !source.contains("[ID: MTLTexture]") {
            violations.append(
                "MetalGridTextureCore/MetalGridTextureCache.swift: cache storage and residency policy must use the generic ID"
            )
        }
        if !source.contains("func uploadVisible(wanted: [ID], provideImage: (ID) -> CGImage?)") {
            violations.append(
                "MetalGridTextureCore/MetalGridTextureCache.swift: upload seam must not re-specialize to a photo-domain ID"
            )
        }
        if FileManager.default.fileExists(
            atPath: timelineRoot.appendingPathComponent("MetalGridTextureCache.swift").path)
        {
            violations.append(
                "TimelineFeature/MetalGridTextureCache.swift: shared generic cache belongs in MetalGridTextureCore")
        }
        if FileManager.default.fileExists(
            atPath: timelineRoot.appendingPathComponent("MetalGridGlyphRasterizer.swift").path)
        {
            violations.append(
                "TimelineFeature/MetalGridGlyphRasterizer.swift: shared glyph contract belongs in MetalGridTextureCore")
        }
        ...
        if !coordinator.contains("MetalGridTextureCache<PhotoUID>") {
            violations.append(
                "TimelineFeature/MetalGridCoordinator.swift: macOS adapter must bind the generic cache to PhotoUID explicitly"
            )
        }
    ...
    func testBackupControllersNeverReadQueueSummaryOnMainActor() throws {
        let controllerFiles = [
            repoRoot.appendingPathComponent("App/FolderBackupController.swift"),
            sourcesRoot.appendingPathComponent(
                "PhotoLibraryBackupAdapter/PhotoLibraryBackupController.swift"
            ),
        ]
        var violations: [String] = []
        ...
        for file in controllerFiles {
            ...
            if code.contains(".summary()") {
                violations.append("\(file.path): queue summary read escaped BackupStatusProjector")
            }
            if code.contains("Task { @MainActor") && code.contains("applyRunnerProgress") {
                violations.append("\(file.path): progress callback creates a MainActor task per tick")
            }
        }
        ...
        XCTAssertTrue(
            violations.isEmpty,
            """
            Backup UI controller bypassed the off-main status projection boundary:
            \(violations.joined(separator: "\n"))
            """
        )
    }
    ...
    private func containsIgnoringWhitespace(_ token: String, in source: String) -> Bool {
        source.filter { !$0.isWhitespace }.contains(token.filter { !$0.isWhitespace })
    }
