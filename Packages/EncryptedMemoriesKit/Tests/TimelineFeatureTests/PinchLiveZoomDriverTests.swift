...
@Suite struct PinchLiveZoomDriverTests {
    ...
    private func settle(_ d: inout PinchLiveZoomDriver, maxTicks: Int = 600) -> Int {
        var ticks = 0
        while !d.isCommitted && ticks < maxTicks {
            d.advance(dt: 1.0 / 60.0)
            ticks += 1
        }
        return ticks
    }

    // One gesture can chain across adjacent detents without a reset or lift.
    ...
    // Reversing direction chains across detents without a reset.
    ...
    // Slow scrubbing tracks the finger within the active segment.
    ...
    // A stationary input leaves both update and tick as no-ops.
    ...
    // Crossing a detent resets segmentQ continuously.
    @Test func segmentQResetsAtDetentCrossing() {
        var d = started(start: 3)
        let before = d.update(continuousLevel: 2.05, dt: 0.05)  // interval [3 to 2], near the L2 detent
    ...
    // Adjacent segments share the same detent endpoint.
    @Test func seamSharesTheCrossedDetent() {
        var d = started(start: 3)
        let before = d.update(continuousLevel: 2.01, dt: 0.05)  // [3 to 2], q≈1, so shows the L2 (target) detent
        #expect(before.segmentTarget == 2)
        let after = d.update(continuousLevel: 1.80, dt: 0.05)  // [2 to 1], q≈0, so shows the L2 (source) detent
        #expect(after.segmentSource == 2)  // same detent (2) on both sides of the crossing
    }

    // Reversing across a segment boundary remains stable.
    @Test func reversalAcrossBoundaryIsStable() {
        var d = started(start: 3)
        _ = d.update(continuousLevel: 1.70, dt: 0.05)  // [2 to 1] q=0.30
        #expect(d.segmentSource == 2 && d.segmentTarget == 1)
        _ = d.update(continuousLevel: 1.95, dt: 0.05)  // reverse toward L2, still [2 to 1] q≈0.05
        #expect(d.segmentSource == 2 && d.segmentTarget == 1)
        let flipped = d.update(continuousLevel: 2.20, dt: 0.05)  // crossed back above L2, so flips to [3 to 2]
    ...
    // A fast flick reaches the final segment in one update.
    @Test func fastFlickJumpsToFinalSegment() {
        var d = started(start: 3)
        let out = d.update(continuousLevel: 0.5, dt: 1.0 / 120.0)  // L3 to ~L0.5 in one event
    ...
    // Mid-segment release settles to the nearest detent.
    @Test func releaseMidSegmentSettlesNearestDetent() {
        var near0 = started(start: 3)
        _ = near0.update(continuousLevel: 1.7, dt: 0.05)  // [2 to 1] q=0.3 (closer to L2)
        ...
        _ = near1.update(continuousLevel: 1.3, dt: 0.05)  // [2 to 1] q=0.7 (closer to L1)
    ...
    // Release settle uses velocity and does not snap instantly.
    @Test func releaseSettleIsNotInstant() {
        var d = started(start: 3)
        _ = d.update(continuousLevel: 1.4, dt: 0.1)  // [2 to 1] q=0.6, modest velocity
        _ = d.release()
    ...
    // A large advance finishes a settle in one step.
    @Test func largeAdvanceForceFinishesSettle() {
        var d = started(start: 3)
        _ = d.update(continuousLevel: 1.3, dt: 0.1)  // [2 to 1] q=0.7
        _ = d.release()
        d.advance(dt: 10)
        #expect(d.isCommitted && d.finalLevel == 1)
    }

    // The chain remains inside its eligible band.
    ...
    // A sub-threshold nudge does not engage a segment.
    @Test func subDeadbandHoldsStart() {
        var d = started(start: 3)
        let out = d.update(continuousLevel: 2.995, dt: 0.1)  // |Δ| 0.005 < 0.02
        #expect(!out.hasSegment)
        #expect(abs(d.segmentQ) < 1e-9)  // start L3 is the source end, so q=0
    }

    // Release before movement commits the start level.
    ...
    // A short directional pinch below the dead band completes one adjacent step.
    ...
    // An interior start chains in both directions.
    ...
    // A midpoint release chooses the target detent on ties.
    @Test func releaseAtExactHalfTieBreaksToTarget() {
        var d = started(start: 3)
        _ = d.update(continuousLevel: 1.5, dt: 0.05)  // [2 to 1], q = 2 − 1.5 = 0.5 exactly
        #expect(abs(d.segmentQ - 0.5) < 1e-9)
        #expect(d.release() == 1)  // tie, so target
        settle(&d)
        #expect(d.finalLevel == 1)
    }

    // Release at a chain extreme settles the boundary detent.
    @Test func releaseAtChainExtremeSettlesBoundary() {
        var d = started(start: 3)
        _ = d.update(continuousLevel: 0.1, dt: 0.05)  // chained down to [1 to 0], q=0.9 (near L0)
    ...
    // A degenerate band is inert and never chains.
    ...
    // The low-pass filter does not smear across a detent crossing.
    @Test func lowPassResetsAtCrossing() {
        var t = PinchLiveZoomDriver.Tunables()
        t.displayQLowPassAlpha = 0.25
        var d = PinchLiveZoomDriver(tuning: t)
        d.begin(startLevel: 3, chainLo: 0, chainHi: 3)
        _ = d.update(continuousLevel: 2.05, dt: 0.05)  // [3 to 2] near the L2 detent
    ...
    // Reset returns the driver to idle for reuse.
