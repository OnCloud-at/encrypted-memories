...
/// Pins the reference-size mapping and the column-count round trip at the reference width.
@Suite struct GridSizePolicyTests {
    ...
    // (the ε-nudge defeats the FP floor-truncation that would otherwise drop L2 to 6 / L5 to 29).
    @Test func referenceSlotSideRoundTripsAtReferenceWidth() {
        ...
        for m in levels {
            ...
            #expect(
                cols == m.nominalColumns,
                "level \(m.levelID): \(cols) ≠ nominalColumns \(m.nominalColumns) at W_ref \(w)")
        }
    }

    // Strictly monotone-decreasing sizes to unambiguous density ladder (zoom in = smaller tiles).
    ...
    // constant size, so column count is non-decreasing in width and strictly grows on a wide enough viewport
    ...
    @Test func maxColumnsCapBindsWithoutResize() {
        ...
        let capped = SquareTileGridEngine.columnsForFixedSide(
            side: m.referenceSlotSide, gap: m.gap, width: 4000, maxColumns: 4)
