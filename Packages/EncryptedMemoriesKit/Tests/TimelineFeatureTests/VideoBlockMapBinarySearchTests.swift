...
struct VideoBlockMapBinarySearchTests {
    ...
    @Test func rangeStartsInsideABlock() {
        ...
        #expect(
            map.slices(offset: 150, length: 30) == [
                BlockSlice(blockIndex: 2, inBlock: ByteRange(lower: 50, upper: 80), fileOffset: 150)
            ])
    }
    ...
    @Test func emptyBlocksAreIgnored() {
        ...
        for (offset, length) in [(0, 300), (90, 30), (100, 100), (199, 5), (0, 1)] {
            #expect(
                withHoles.slices(offset: offset, length: length)
                    == linearSlices(withHoles, offset: offset, length: length))
        }
    ...
    @Test func forwardBlocksMatchesLinearFilter() {
        ...
        for _ in 0..<200 {
            ...
            let sizes: [(index: Int, clearSize: Int)] = (1...blockCount).map {
                ($0, Int.random(in: 0...400, using: &rng))
            }
            let map = VideoBlockMap(blockSizes: sizes)
            for _ in 0..<8 {
                ...
                #expect(
                    map.forwardBlocks(afterClearOffset: offset, count: count)
                        == linearForwardBlocks(map, afterClearOffset: offset, count: count))
            }
        }
    }
