...
@Suite struct TimelineSearchFilterTests {
    ...
    @Test func searchMatchesSectionTitleAndKeepsWholeSection() {
        ...
        let result = TimelineSearch.filter(sections, query: "Feb")
    ...
    @Test func searchMatchesItemMediaTypeAndFiltersWithinSection() {
        let sections = makeSections()
        let result = TimelineSearch.filter(sections, query: "video")
        #expect(result.count == 1)
    ...
    @Test func smartSearchMatchesFavoritesFromContext() {
        ...
        let result = TimelineSearch.filter(
            sections,
            query: "Favoriten",
    ...
    @Test func smartSearchMatchesServerSideTags() {
        let sections = makeSections()
        let raw = TimelineSearch.filter(sections, query: "raw")
        let selfie = TimelineSearch.filter(sections, query: "selfies")
    ...
    @Test func smartSearchRespectsActiveServerFilterWhenTagsAreMissingLocally() {
        let sections = [
            TimelineSection(
                id: "filtered", date: baseDate, title: "",
                items: [
                    PhotoItem(
                        uid: PhotoUID(volumeID: "v", nodeID: "server-filtered-a"),
    ...
    @Test func smartSearchCanCombineKindAndDate() {
        let sections = makeSections()
        let result = TimelineSearch.filter(sections, query: "video 2026 februar")
    ...
    @Test func semanticMatchesWidenLexicalResultsInTimelineOrder() {
        ...
        let result = TimelineSearch.filter(sections, query: "video", semanticMatches: semantic)
    ...
    @Test func semanticMatchesDoNothingWithoutAQuery() {
        let sections = makeSections()
        let semantic: Set<PhotoUID> = [PhotoUID(volumeID: "v", nodeID: "scan-node")]
        let result = TimelineSearch.filter(sections, query: "   ", semanticMatches: semantic)
    ...
    private func makeSections() -> [TimelineSection] {
        [
            TimelineSection(
                ...
                items: [
                    PhotoItem(
                        uid: PhotoUID(volumeID: "v", nodeID: "portrait-node"),
                        captureTime: baseDate,
                        mediaType: "image/jpeg"),
                    PhotoItem(
                        uid: PhotoUID(volumeID: "v", nodeID: "video-node"),
