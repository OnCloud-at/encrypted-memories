...
/// Compares the optimized date-marker pass with a per-item reference implementation.
@Suite struct DateMarkerEquivalenceTests {
    ...
    /// Reference implementation: emit a marker when the selected date components change.
    private func referenceMarkers(
        _ items: [PhotoItem],
    ...
    @Test func matchesReferenceAcrossFixturesAndGranularities() {
        ...
        // Non-monotonic: a month recurs non-adjacently to a fresh marker must reappear.
        let nonMonotonic = [
        ...
        for y in 2022...2025 {
            for m in 1...12 {
                for d in stride(from: 1, through: 27, by: 9) {
                    for _ in 0..<3 {
                        big.append(item(idx, date(y, m, d)))
                        idx += 1
                    }
                }
            }
    ...
    @Test func itemsAndSectionsOverloadsAgree() {
        ...
        let viaItems = MetalGridProductionAdapter.dateMarkers(
            items: items, granularity: .month,
            calendar: Self.calendar, locale: Self.locale)
        let viaSections = MetalGridProductionAdapter.dateMarkers(
            sections: [section], granularity: .month,
            calendar: Self.calendar, locale: Self.locale)
    ...
    @Test func emptyItemsProduceNoMarkers() {
        #expect(
            MetalGridProductionAdapter.dateMarkers(
                items: [], granularity: .month,
                calendar: Self.calendar, locale: Self.locale
            ).isEmpty)
    }
}
