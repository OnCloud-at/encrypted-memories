...
/// The zoom anchor is an item plus a local slot fraction, not a raw scroll offset.
/// Geometry changes rebase from that anchor and preserve its viewport position.
@Suite struct ZoomAnchorPreservationTests {
    ...
    private func capture(
        _ e: SquareTileGridEngine, level: Int, scrollY: CGFloat
    )
        -> (anchor: GridZoomAnchor, flatIndex: Int, localFraction: CGPoint)?
    {
        ...
        let anchor = GridZoomAnchor(
            flatIndex: a.flatIndex, viewportPoint: viewportPoint,
    ...
    @Test func zoomPreservesAnchorItem() {
        let e = singleSection()
        guard let cap = capture(e, level: 2, scrollY: 9000) else {
            Issue.record("no anchor")
            return
        }
        for x in [CGFloat(2.0), 2.4, 3.0, 3.6, 4.2, 5.0] {
            ...
            guard let slot = plan.visibleSlots.first(where: { $0.index == cap.flatIndex }) else {
                Issue.record("anchor item not visible at \(x)")
                continue
            }
    ...
    @Test func zoomDoesNotJumpToDifferentTimelinePosition() {
        ...
        for x in [CGFloat(2.0), 2.5, 3.0, 4.0, 5.0] {
            ...
            guard let slot = plan.visibleSlots.first(where: { $0.index == cap.flatIndex }) else {
                Issue.record("anchor jumped off-screen at \(x)")
                continue
            }
    ...
    @Test func sectionAwareAnchorPreservation() {
        ...
        guard let cap = capture(e, level: 2, scrollY: scrollY) else {
            Issue.record("no anchor")
            return
        }
        ...
        for x in [CGFloat(2.0), 2.6, 3.3, 4.0, 4.8] {
            let plan = e.zoomFramePlan(continuousLevel: x, viewportSize: viewport, anchor: cap.anchor, overscan: 400)
            guard let slot = plan.visibleSlots.first(where: { $0.index == cap.flatIndex }) else {
                Issue.record("section anchor not visible at \(x)")
                continue
            }
            let landedY = slot.viewportRect.minY + cap.localFraction.y * slot.viewportRect.height
    ...
    @Test func scrollOffsetRebasedWhenMetricsChange() {
        ...
        guard let cap = capture(e, level: 2, scrollY: startScrollY) else {
            Issue.record("no anchor")
            return
        }
        let rebased = e.anchoredScrollOffset(
            flatIndex: cap.flatIndex, localFraction: cap.localFraction,
        ...
        #expect(abs((anchorContentY - rebased.y) - viewportPoint.y) < 1.0)  // rebased offset is correct
        #expect(abs((anchorContentY - startScrollY) - viewportPoint.y) > 50.0)  // raw offset is wrong (jump)
    }
    ...
    @Test func visibleSlotsStableAroundAnchorDuringPinch() {
        ...
        for step in 0...20 {
            let x = 2.0 + CGFloat(step) * 0.1  // 2.0 to 4.0 in fine steps
