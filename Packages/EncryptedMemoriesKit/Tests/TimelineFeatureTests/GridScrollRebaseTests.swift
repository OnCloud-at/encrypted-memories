...
@Suite struct GridScrollRebaseTests {
    // Arms only when the gesture or anchored scroll differs from the legal settled scroll.
    @Test func armsOnlyWhenSourceDiffersFromTarget() {
        #expect(GridScrollRebase.shouldArm(fromY: 100, toY: 100) == false)  // normal commit needs no bridge
        #expect(GridScrollRebase.shouldArm(fromY: 100, toY: 100.4) == false)  // imperceptible change is instant
        #expect(GridScrollRebase.shouldArm(fromY: 420, toY: 300) == true)  // edge clamp needs a bridge
        #expect(GridScrollRebase.shouldArm(fromY: 0, toY: 90) == true)
    }

    // Progress interpolates from source to target monotonically, staying within [from, to] without bouncing.
    @Test func interpolatesMonotonicallyNoBounce() {
        let from: CGFloat = 420
        let to: CGFloat = 300  // a downward clamp (zoom-out shrank content)
    ...
    // The bridge's final frame equals the canonical settled scroll exactly.
    ...
    @Test func progressClampsToUnitInterval() {
        ...
        #expect(GridScrollRebase.progress(start: 10, now: 100) == 1)  // way past is clamped
        #expect(GridScrollRebase.progress(start: 10, now: 5) == 0)  // before start is clamped
