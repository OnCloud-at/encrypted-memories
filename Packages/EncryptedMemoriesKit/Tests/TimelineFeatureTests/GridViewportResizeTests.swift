...
@Suite struct GridViewportResizeTests {
    ...
    private func rebase(
        _ e: SquareTileGridEngine, oldFrame: CGRect, newFrame: CGRect, scrollY: CGFloat,
        level: Int, phase: Int?, bottomPinned: Bool = false, anchorFractionY: CGFloat = 0.5
    ) -> GridViewportResizeResult {
        e.rebasedScrollOffsetForViewportChange(
    ...
    private func anchorAt(
        _ e: SquareTileGridEngine, width: CGFloat, scrollY: CGFloat, vh: CGFloat, fraction: CGFloat = 0.5, level: Int,
        phase: Int?
    ) -> Int? {
        e.anchorItem(
            nearContentPoint: CGPoint(x: width / 2, y: scrollY + vh * fraction), level: level, width: width,
            columnPhase: phase)?.flatIndex
    }
    private func visibleSet(
        _ e: SquareTileGridEngine, width: CGFloat, vh: CGFloat, scrollY: CGFloat, level: Int, phase: Int?
    ) -> Set<Int> {
        Set(
            e.framePlan(
                level: level, viewportSize: CGSize(width: width, height: vh),
    ...
    @Test func verticalResizeUsesNormalizedViewportAnchor() {
        let e = engine()
        let old = CGRect(x: 0, y: 0, width: 1000, height: 1000)
        let new = CGRect(x: 0, y: 200, width: 1000, height: 800)
        for level in [0, 2, 3] {
            for scrollY in [CGFloat(3000), 6000] {
                ...
                #expect(
                    anchorAt(e, width: 1000, scrollY: r.newScrollY, vh: 800, level: level, phase: nil) == centerBefore,
                    "centre anchor not preserved (L\(level) y\(scrollY))")
                let strictTop = rebase(
                    e, oldFrame: old, newFrame: new, scrollY: scrollY, level: level, phase: nil, anchorFractionY: 0
                ).newScrollY
                let strictBottom = rebase(
                    e, oldFrame: old, newFrame: new, scrollY: scrollY, level: level, phase: nil, anchorFractionY: 1
                ).newScrollY
                #expect(abs(r.newScrollY - scrollY) > eps, "must rebase, not reuse raw scrollY")
                #expect(
                    r.newScrollY > min(strictTop, strictBottom) + eps
                        && r.newScrollY < max(strictTop, strictBottom) - eps,
                    "normalized anchor must lie BETWEEN strict-top and strict-bottom pins")
            }
        }
    }

    // Bottom-edge shrink rebases scrollY around the centre anchor, clips lower content, and avoids a jump.
    ...
    // Bottom-edge expansion reveals more content smoothly without a jump.
    @Test func bottomEdgeExpandRevealsAndRebasesContinuously() {
        let e = engine()
        let old = CGRect(x: 0, y: 200, width: 1000, height: 800)
        let new = CGRect(x: 0, y: 0, width: 1000, height: 1000)
        let centerBefore = anchorAt(e, width: 1000, scrollY: 6000, vh: 800, level: 2, phase: nil)!
        let r = rebase(e, oldFrame: old, newFrame: new, scrollY: 6000, level: 2, phase: nil)
        #expect(r.anchorGlobalIndex == centerBefore)
        #expect(
            visibleSet(e, width: 1000, vh: 1000, scrollY: r.newScrollY, level: 2, phase: nil).count
                > visibleSet(e, width: 1000, vh: 800, scrollY: 6000, level: 2, phase: nil).count, "expand reveals more")
    }

    // Top-edge shrink holds the centre and clips upper rows after rebasing.
    @Test func topEdgeShrinkClipsAndRebasesContinuously() {
        let e = engine()
        let old = CGRect(x: 0, y: 0, width: 1000, height: 1000)
        let new = CGRect(x: 0, y: 0, width: 1000, height: 800)  // maxY ↓
        let centerBefore = anchorAt(e, width: 1000, scrollY: 6000, vh: 1000, level: 2, phase: nil)!
        let r = rebase(e, oldFrame: old, newFrame: new, scrollY: 6000, level: 2, phase: nil)
        #expect(r.anchorGlobalIndex == centerBefore)
        #expect(
            anchorAt(e, width: 1000, scrollY: r.newScrollY, vh: 800, level: 2, phase: nil) == centerBefore,
            "centre jumped")
    }

    // Top-edge expansion holds the centre and reveals more content.
    @Test func topEdgeExpandRevealsAndRebasesContinuously() {
        let e = engine()
        let old = CGRect(x: 0, y: 0, width: 1000, height: 800)
        let new = CGRect(x: 0, y: 0, width: 1000, height: 1000)  // maxY ↑
        let centerBefore = anchorAt(e, width: 1000, scrollY: 6000, vh: 800, level: 2, phase: nil)!
        let r = rebase(e, oldFrame: old, newFrame: new, scrollY: 6000, level: 2, phase: nil)
        #expect(r.anchorGlobalIndex == centerBefore)
        #expect(
    ...
    // An unchanged width leaves width-derived metrics unchanged.
    @Test func pureHeightResizeDoesNotChangeGridMetrics() {
        ...
        let r = rebase(
            e, oldFrame: CGRect(x: 0, y: 0, width: 1000, height: 1000),
            newFrame: CGRect(x: 0, y: 200, width: 1000, height: 800), scrollY: 6000, level: 2, phase: nil)
        #expect(r.newContentSize.height == e.contentSize(level: 2, width: 1000).height)
        let after = e.resolvedMetrics(level: 2, width: 1000)
        #expect(
            before.columns == after.columns && abs(before.slotSide - after.slotSide) < eps,
            "pure height must not change width-derived metrics")
    }

    // A fixed-column width change fills the grid without a gutter; the column count is
    ...
    @Test func pureWidthResizeFillsWidthScalesTileNoReflow() {
        ...
        let r = rebase(e, oldFrame: old, newFrame: new, scrollY: 6000, level: 2, phase: nil)
        #expect(r.anchorGlobalIndex == centerBefore)
        // Fixed-columns, so no reflow, so the centre item is preserved exactly (no horizontal neighbour drift).
        let after = anchorAt(e, width: 1400, scrollY: r.newScrollY, vh: 800, level: 2, phase: nil)!
        #expect(
            after == centerBefore,
            "centre item must be preserved exactly on a fixed-columns resize (\(after) vs \(centerBefore))")
    }

    // A combined width and height change uses new-width metrics and preserves the anchor.
    @Test func combinedResizeRebasesFromLogicalAnchor() {
        let e = engine()
        let old = CGRect(x: 0, y: 0, width: 1000, height: 800)
        let new = CGRect(x: 0, y: 0, width: 1300, height: 1000)
        let centerBefore = anchorAt(e, width: 1000, scrollY: 6000, vh: 800, level: 2, phase: nil)!
        let r = rebase(e, oldFrame: old, newFrame: new, scrollY: 6000, level: 2, phase: nil)
        #expect(r.anchorGlobalIndex == centerBefore)
        ...
        #expect(
            ...
            "centre item must stay within one row of the preserved anchor (got \(after) vs \(centerBefore))")
    }

    // A sidebar toggle uses the same width-resize path.
    @Test func sidebarToggleUsesWidthResizeRebasePath() {
        let e = engine()
        let wide = CGRect(x: 0, y: 0, width: 1280, height: 860)
        let narrow = CGRect(x: 300, y: 0, width: 980, height: 860)
    ...
    @Test func bottomPinnedResizeStaysBottomPinned() {
        ...
        let new = CGRect(x: 0, y: 0, width: 1400, height: 800)
        let r = rebase(
            e, oldFrame: CGRect(x: 0, y: 0, width: 1000, height: 800), newFrame: new, scrollY: 12345, level: 2,
            phase: nil, bottomPinned: true)
    ...
    @Test func nonBottomResizeDoesNotBecomeBottomPinned() {
        let e = engine()
        let old = CGRect(x: 0, y: 0, width: 1000, height: 800)
        let new = CGRect(x: 0, y: 0, width: 1200, height: 800)
    ...
    @Test func resizeGapConsistency() {
        ...
        let new = CGRect(x: 0, y: 0, width: 1400, height: 800)
        let r = rebase(
            e, oldFrame: CGRect(x: 0, y: 0, width: 1000, height: 800), newFrame: new, scrollY: 6000, level: 2,
            phase: nil)
        let m = e.resolvedMetrics(level: 2, width: new.width)
        let plan = e.framePlan(
            level: 2, viewportSize: CGSize(width: new.width, height: new.height),
            scrollOffset: CGPoint(x: 0, y: r.newScrollY), overscan: 0, columnPhase: nil)
        let row = Dictionary(grouping: plan.visibleSlots, by: { $0.row }).values.first { $0.count == m.columns }!.sorted
        { $0.column < $1.column }
    ...
    @Test func appleResizeReferenceRegression() {
        ...
        for newVH in stride(from: CGFloat(1000), through: 600, by: -100) {  // bottom edge rising in steps
            ...
            #expect(
                anchorAt(e, width: 1000, scrollY: r.newScrollY, vh: newVH, level: 2, phase: nil) == centerStart,
                "centre drifted at vh \(newVH)")
            ...
            lastScrollY = r.newScrollY
            scrollY = r.newScrollY
            vh = newVH
        }
