...
/// Structural assertions enforce the Metal grid engine contract.
@Suite struct MetalGridContractGuardTests {
    ...
    private func packageRoot() -> URL {
        ...
        return u.appendingPathComponent("Packages/EncryptedMemoriesKit")
    }
    private func sourceDirs() -> [URL] {
        ["TimelineFeature", "GridCore", "MetalRenderingCore"].map {
            packageRoot().appendingPathComponent("Sources/\($0)")
        }
    }
    ...
    @Test func productionTimelineIsMetalGridOnlyGuard() {
        ...
        #expect(
            src("MetalProductionGridView.swift").contains("MetalGridScrollHost"),
            "production grid must use the Metal host")
    }

    @Test func noLegacyGridSymbolsGuard() {
        let all = allProductionSource()
        for symbol in [
            "PhotoGridView", "PhotoGridItem", "JustifiedCollectionLayout", "GridZoomMath",
    ...
    @Test func engineOwnsSlotGeometryGuard() {
        ...
        #expect(
            src("MetalGridCoordinator.swift").contains("engine.framePlan"),
            "coordinator must source geometry from the engine")
    }
    ...
    @Test func resizeDoesNotUseZoomTransactionGuard() {
        ...
        #expect(
            !resize.contains("beginZoomTransaction") && !resize.contains("GridZoomCommitBridge.")
                && !resize.contains("GridZoomTransaction("))
    ...
    // Normal AppKit scroll and rubber-band own the clip origin. The settled draw path may render at the
    ...
    @Test func settledDrawDoesNotFightNativeScrollElasticityGuard() throws {
        ...
        guard let start = coordinator.range(of: "private func drawEngineFrame"),
            let end = coordinator[start.lowerBound...].range(of: "    nonisolated static func viewportDrawSlots")
        else {
        ...
        #expect(
            !body.contains("clip.scroll(to:"), "settled draw must not clamp the NSClipView during native rubber-band")
    ...
    @Test func pinchUsesCursorAnchorGuard() {
        ...
        for phase in [nil, Int(3)] {
            ...
            let displayed = e.anchorItem(nearContentPoint: cursorContent, level: 2, width: width, columnPhase: phase)!
                .flatIndex
            let tx = e.beginZoomTransaction(
                cursorContentPoint: cursorContent, viewportPoint: cursorVP, level: 2, width: width, columnPhase: phase)!
            #expect(
                tx.anchorGlobalIndex == displayed,
                "pinch must anchor the item under the cursor (phase \(String(describing: phase)))")
        }
        #expect(
            src("MetalGridCoordinator.swift").containsCodeFragmentIgnoringWhitespace(
                "beginZoomTransaction(cursorContentPoint:"
            ),
            "live zoom uses the cursor content point")
    }
    ...
    @Test func noAspectOuterLayoutGuard() {
        ...
        #expect(
            !src("SquareTileGridEngine.swift").contains("mediaAspect")
                && !src("SquareTileGridEngine.swift").contains("displayMode"))
    }
    ...
    @Test func coordinatorUsesInjectedViewportProfileWithRegularDefault() {
        ...
        #expect(
            coord.containsCodeFragmentIgnoringWhitespace(
                "SquareTileGridEngine(sectionCounts: dataSource.sectionCounts, profile: gridProfile"
            ))
