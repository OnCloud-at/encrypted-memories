...
@Suite struct SquareTileGridEngineTests {
    ...
    private func settledPlan(level: Int, scrollY: CGFloat = 0, count: Int = 2000) -> GridFramePlan {
        engine(count).framePlan(
            level: level, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: scrollY), overscan: 0)
    }
    ...
    // Slot rects are square for every visible slot at every level.
    ...
    // Media aspect cannot affect the slot rect: the engine takes no aspect input. Content fitting lives
    ...
    // Gap is a first-class metric: adjacent slots are exactly `pitch` apart.
    ...
    // `pitch` equals `slotSide + gap` in the plan and adjacency checks.
    ...
    // Fixed-column width-fill keeps full rows leading-aligned at column zero
    ...
    @Test func visibleQueryIsLeadingAlignedAndFillsWidth() {
        for level in 0..<engine().levelCount {
            let plan = settledPlan(level: level, scrollY: midScroll(level: level))  // mid-library to full rows
    ...
    // Fixed-column window resize fills the width at every width without a gutter; the column
    ...
    @Test func windowResizeFillsWidthFixedColumnsScalesTile() {
        ...
        for w in [CGFloat(600), 900, 1280, 1920] {
            let plan = e.framePlan(
                level: 2, viewportSize: CGSize(width: w, height: 900), scrollOffset: CGPoint(x: 0, y: 3000), overscan: 0
            )
        ...
        for c in columnsSeen {
            #expect(
                c == nominal,
                "FIXED-COLUMNS: the count holds at \(nominal) across widths (resize scales, never reflows)")
        }
        #expect(
            sides.first! < sides.last!, "the tile must SCALE with width (fixed-columns, not the old re-column reflow)")
        for i in 1..<sides.count { #expect(sides[i] >= sides[i - 1], "tile size monotone non-decreasing in width") }
    }

    // Hit testing uses the square slot rect; a point in the slot corner that an aspect-fit inner rect
    ...
    @Test func hitTestingUsesSlotRect() {
        ...
        // Center hits.
        #expect(
            e.hitTest(contentPoint: CGPoint(x: slot.slotRect.midX, y: slot.slotRect.midY), level: 1, width: width)?
                .index == slot.index)
    ...
    // Zoom changes grid metrics, never media geometry. Slots stay
    ...
    @Test func zoomUsesMetrics() {
        ...
        #expect(abs(e.apparentSlotSide(at: 2, width: w) - filledSide(2)) < eps)
        let mid = e.apparentSlotSide(at: 2.5, width: w)
        #expect(mid < filledSide(2) && mid > filledSide(3))  // between detents
        let anchor = GridZoomAnchor(
            flatIndex: 1000, viewportPoint: CGPoint(x: 700, y: 450),
        ...
        #expect(outPlan.columns > inPlan.columns)  // zoom out to more columns
        for s in outPlan.visibleSlots { #expect(abs(s.slotRect.width - s.slotRect.height) < eps) }  // still square
    }

    // Continuous zoom-out keeps the lens leading-aligned with a
    ...
    // Anchor preservation keeps the item under the same viewport point as metrics change.
    @Test func anchorPreservation() {
        let e = engine()
        let anchorPoint = CGPoint(x: 700, y: 450)
        let anchor = GridZoomAnchor(
            flatIndex: 1200, viewportPoint: anchorPoint,
            contentFractionY: 0.5, relInCell: CGPoint(x: 0.5, y: 0.5))
        for x in [CGFloat(2.0), 2.4, 3.0, 3.7] {
            ...
            guard let slot = plan.visibleSlots.first(where: { $0.index == 1200 }) else {
                Issue.record("anchor item not visible at level \(x)")
                continue
            }
    ...
    @Test func rowColumnRoundTrips() {
        ...
        for flat in [0, 1, 50, 137, 999, 1999] {
            guard let loc = e.locate(flatIndex: flat, level: 2, width: width) else {
                Issue.record("no loc for \(flat)")
                continue
            }
