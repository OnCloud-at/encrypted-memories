...
@Suite struct MetalGridUploadDedupTests {
    @Test func sameUIDNotUploadedTwiceConcurrently() {
        ...
        #expect(lru.selectUploads(wanted: [a, b, c]) == [c])  // a,b deduped to only c
        lru.completeUpload(a, cost: 1)
        #expect(lru.selectUploads(wanted: [a, b, c]) == [])  // a resident, b & c in flight to none
    }
}
...
@Suite struct MetalGridStreamingPolicyTests {
    ...
    @Test func macOSDefaultBudgetKeepsDenseScrollNeighborhoodResidentButByteBounded() {
        ...
        // Per-frame upload bytes remain bounded for a 60 Hz frame budget.
    ...
    @Test func iOSTierBudgetsStayMoreConservativeThanMacOS() {
        ...
        for policy in [
            UIKitMetalGridTexturePolicies.compact,
    ...
    @Test func denseLevelAwarePoliciesBindOnBytesBeforeCountAtUploadFloor() {
        ...
        // The count cap must not fill before the byte budget, or scrolling can churn despite available bytes.
        ...
        for policy in policies {
            #expect(
                policy.budget.maxCachedTextures * floorTextureBytes >= policy.budget.maxResidentBytes,
...
@Suite struct MetalGridGlyphRasterizerTests {
    @Test func glyphRequestCacheKeyIncludesSymbolWeightSizeAndColor() {
        ...
        #expect(base != MetalGridGlyphRequest(text: "1", pixelSize: 44, aspectRatio: 0.62, weight: .bold, color: white))

        let text = MetalGridGlyphRequest(text: "1", aspectRatio: 0.62, color: white)
        #expect(text == MetalGridGlyphRequest(text: "1", aspectRatio: 0.62, color: white))
        #expect(text != MetalGridGlyphRequest(text: "2", aspectRatio: 0.62, color: white))
        #expect(text != MetalGridGlyphRequest(text: "1", aspectRatio: 0.66, color: white))
        #expect(
            text
                != MetalGridGlyphRequest(
                    text: "1", aspectRatio: 0.62, textStyle: .monospaced, color: white))
        #expect(text.canvasPixelWidth == 37)
    }
}

@Suite @MainActor struct MetalGridTextureCacheByteBudgetTests {
    /// A decoded-image stand-in: a solid 64×64 CGImage to 64·64·4 = 16,384 texture bytes.
    ...
    @Test func residencySaturatedIsSetOnlyWhenAPinnedVisibleUploadCannotFit() throws {
        ...
        // Frame 3: a pinned visible upload beyond what the pinned floor can ever hold is saturation
    ...
    @Test func effectiveCapShrinksDenseUploadSizeAndByteCost() throws {
        guard
            let cache = Self.makeCache(
                budget: GridTextureBudget(
                    maxUploadsPerFrame: 10, maxUploadBytesPerFrame: 1_048_576,
                    maxCachedTextures: 1000, maxResidentBytes: 1_048_576, overscanFraction: 1.0
                )), let image = Self.makeImage(side: 64)
        else { return }  // 64px source (cap is also 64)

        // Dense level: the coordinator lowers the effective cap to 32 px to a 64px source uploads a 32×32
    ...
    @Test func largeLevelEffectiveCapRetainsFullQualityUploadSize() throws {
        ...
        // identical to the fixed-size behaviour at large levels.
    ...
    @Test func undersizedResidentUpgradesInPlaceWhenCapGrows() throws {
        ...
        // and the resident-byte ledger swaps 32² to 64² without changing the resident count.
        cache.setEffectiveMaxTexturePixels(64)
        cache.beginFrame(pinned: [item])
        cache.uploadVisible(wanted: [item].filter { !cache.isResident($0) }) { _ in image }  // resident to nothing new
    ...
    @Test func upgradeRespectsPerFrameUploadByteBudgetAndSignalsPending() throws {
        ...
        #expect(cache.upgradesThisFrame == 1)
        #expect(cache.uploadBytesThisFrame == 16_384)
        #expect(cache.pendingUpgradesThisFrame)  // more to do to coordinator keeps ticking
        #expect(cache.residentBytes <= cache.residentByteBudget)
    }
    ...
    @Test func glyphSizingIgnoresEffectiveCap() throws {
        ...
        // Even with a tiny effective cap, the reusable glyph source stays sharp at its requested pixel size.
        // GridThumbnailOverlayPolicy scales the rendered quad; raster resolution must not fall with it.
    ...
    @Test func scrolledAwayTexturesEvictSoNewWindowStaysWithinByteBudget() throws {
        ...
        // A new window pins new items; items outside that window become evictable.
...
@Suite struct MetalGridRendererSmokeTests {
    ...
    func rendererBuildsAndShaderCompiles() throws {
        let device = try #require(MTLCreateSystemDefaultDevice())
        let renderer = try #require(MetalGridRenderer(device: device))
        #expect(renderer.hasAuxiliaryPipelines)
    }
}
...
@Suite struct MetalGridDiagnosticsTests {
    @Test func statsReflectRealVsPlaceholderCounts() {
        ...
        let counts = MetalGridStats.counts(visibleCount: 4, overscanCount: 0, drawnUIDs: drawn) {
            resident.contains($0)
        }
