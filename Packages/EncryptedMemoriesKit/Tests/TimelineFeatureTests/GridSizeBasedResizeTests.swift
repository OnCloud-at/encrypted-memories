...
@Suite struct GridSizeBasedResizeTests {
    ...
    private func engine(_ count: Int = 8000) -> SquareTileGridEngine {
    ...
    @Test func fillsWidthFixedColumnsAcrossAWidthSweep() {
        ...
        for level in 0..<e.levelCount {
            ...
            for w in stride(from: CGFloat(500), through: 3000, by: 17) {
                ...
                #expect(
                    m.columns == nominal,
                    "L\(level) w\(w): fixed-columns - the count must hold at \(nominal) (got \(m.columns))")
                let contentWidth = CGFloat(m.columns) * m.pitch - m.gap
                #expect(
                    abs(contentWidth - w) < 1.0,
                    "L\(level) w\(w): grid must FILL the width (gutter \(w - contentWidth))")
                #expect(
                    m.slotSide >= lastSide - 0.001,
                    "L\(level): the tile must SCALE up as width grows, never reflow (w=\(w))")
                lastSide = m.slotSide
            }
        }
    }
    ...
    @Test func columnsFixedTileScalesWhenWider() {
        ...
        for level in 0..<e.levelCount {
            for w in [CGFloat(700), 2560] {
                let m = e.resolvedMetrics(level: level, width: w)
                #expect(
                    abs((CGFloat(m.columns) * m.pitch - m.gap) - w) < 1.0, "L\(level) w\(w): grid must fill the width")
            }
            ...
            #expect(
                ...
                "L\(level): a wider viewport keeps the SAME column count (fixed-columns)")
    ...
    @Test func sidebarToggleScalesNoReflow() {
        ...
        for level in 0..<4 {  // normal photo levels (the sidebar gap applies here)
            ...
            #expect(
                withSidebar.columns == withoutSidebar.columns,
                "L\(level): the sidebar must NOT reflow (same column count)")
            #expect(
                withSidebar.slotSide < withoutSidebar.slotSide,
                "L\(level): the sidebar scales the tile smaller to fill the reduced width")
            #expect(
                abs((CGFloat(withSidebar.columns) * withSidebar.pitch - withSidebar.gap) - (fullWidth - sidebarInset))
                    < 1.0,
    ...
    @Test func clickInTrailingMarginHitsNothing() {
        ...
        #expect(
            e.hitTest(contentPoint: CGPoint(x: marginX, y: 4000), level: level, width: w) == nil,
            "a point right of the last filled column must not hit a slot")
    }

    // Width-fill contract: the grid must fill the width at sizes that expose a trailing gutter.
    @Test func wideWindowHasNoHugeGutter() {
        let e = engine()
        let cases: [(level: Int, width: CGFloat)] = [
            (0, 1700), (0, 2141), (0, 1440), (1, 1440), (1, 2309), (2, 1644), (3, 1418),
        ]
        for c in cases {
    ...
    @Test func pinchCommitSeamHoldsAtNonReferenceWidths() {
        ...
        for w in [CGFloat(1500), 1733, 980, 2200] {  // none is the 1280 reference width
            for level in 0..<e.levelCount {
                ...
                #expect(
                    abs(e.apparentSlotSide(at: CGFloat(level), width: w) - settled) < 0.5,
