...
/// the oldest top-left and newest bottom-right ends; the bottom pin resets to canonical.)
@Suite struct GridLayoutPhaseTests {
    ...
    private func setup(
        target: Int
    ) -> (e: SquareTileGridEngine, tx: GridZoomTransaction, desiredCol: Int, phase: Int, scrollY: CGFloat) {
    ...
    private func phasedPlan(
        _ s: (e: SquareTileGridEngine, tx: GridZoomTransaction, desiredCol: Int, phase: Int, scrollY: CGFloat),
        target: Int
    ) -> GridFramePlan {
        s.e.framePlan(
    ...
    @Test func settledTargetPreservesCursorAnchorColumn() {
        for target in 0..<SquareTileGridEngine.testRegularLevels.count {
            ...
            #expect(
                anchorSlot?.column == s.desiredCol,
    ...
    @Test func commitAnchorHorizontalDeltaIsSmall() {
        ...
        for target in [0, 2, 3, 5, 6] {
            ...
            #expect(
                abs(phased.anchorDelta.width) < pitch,
            ...
            #expect(
                abs(phased.anchorDelta.width) < abs(canonical.anchorDelta.width),
    ...
    @Test func selectedPhotoDoesNotMoveToFarRightOnRelease() {
        let target = 5  // a large zoom-out to the densest level
        let s = setup(target: target)
        ...
        // Without the cursor-aligned phase, the canonical bottom-right layout places the anchor elsewhere.
        let canonicalCol = s.e.framePlan(
            level: target, viewportSize: viewport,
            scrollOffset: CGPoint(x: 0, y: s.scrollY), overscan: 0
        )
        .visibleSlots.first { $0.index == anchor }?.column
        #expect(canonicalCol != s.desiredCol, "canonical phase would move the anchor to a different column")
    }
    ...
    @Test func phaseComputedFromAnchorGlobalIndexAndDesiredColumn() {
        ...
        for desired in [0, 1, cols / 2, cols - 1] {
            ...
            let scrollY = e.anchoredScrollOffset(
                flatIndex: anchor, localFraction: CGPoint(x: 0.5, y: 0.5),
                viewportPoint: cursor, level: target, width: width, columnPhase: phase
            ).y
            let col = e.framePlan(
                level: target, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: scrollY), overscan: 0,
                columnPhase: phase
            )
            .visibleSlots.first { $0.index == anchor }?.column
    ...
    @Test func gridPhasePersistsAfterZoom() {
        ...
        for scrollY in [CGFloat(800), 3000, 7000] {
            ...
            for slot in plan.visibleSlots {
                #expect(
                    slot.column == ((s.phase + slot.index) % cols + cols) % cols,
    ...
    @Test func scrollingAfterPhasedZoomRemainsCoherent() {
        ...
        let s = setup(target: target)
        let pitch = s.e.resolvedMetrics(level: target, width: width).pitch
        let planA = s.e.framePlan(
            level: target, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: 2000), overscan: pitch * 2,
            columnPhase: s.phase)
        let planB = s.e.framePlan(
            level: target, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: 2000 + pitch), overscan: pitch * 2,
            columnPhase: s.phase)
        ...
        for sa in planA.visibleSlots {
            ...
            #expect(
                abs((sa.viewportRect.minY - sb.viewportRect.minY) - pitch) < 0.5,
                "row not coherent on scroll for \(sa.index)")
            checked += 1
        }
