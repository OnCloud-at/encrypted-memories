...
@Suite struct GridProfileRebaseTests {
    ...
    private func rebase(
        ...
        mapping: GridProfileRebaseLevelMapping = .closestVisualMatch
    ) -> GridProfileRebaseResult {
        source.rebasedScrollOffsetForProfileChange(
    ...
    @Test func regularDefaultMapsToCompactClosestNormalLevelAndPreservesAnchor() throws {
        ...
        let after = try #require(
            anchorAt(target, width: 390, height: 900, scrollY: result.newScrollY, level: result.targetLevel))
    ...
    @Test func compactDefaultMapsBackToRegularDefaultAndPreservesAnchor() throws {
        ...
        let after = try #require(
            anchorAt(target, width: 1280, height: 900, scrollY: result.newScrollY, level: result.targetLevel))
    ...
    @Test func visualMappingDoesNotCrossTheOverviewMonthLabelBoundary() {
        ...
        #expect(
            target.metrics(level: result.targetLevel).monthLabels,
    ...
    @Test func preserveLevelIDPolicyIsAvailableForAdapterControlledTransitions() throws {
        ...
        let before = try #require(anchorAt(source, width: 1280, height: 900, scrollY: 6000, level: 3))
        let result = rebase(
            source: source, target: target, oldWidth: 1280, newWidth: 390,
