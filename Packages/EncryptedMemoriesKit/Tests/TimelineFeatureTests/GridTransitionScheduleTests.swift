...
@Suite struct GridTransitionScheduleTests {
    ...
    private func referenceComponents() -> [GridTransitionComponent] {
        func c(_ id: Int, _ area: Double, fd: Int) -> GridTransitionComponent {
            .init(
                id: id, keys: [RelativeSlotKey(dr: fd == 0 ? 0 : (id % 2 == 0 ? -fd : fd), dc: id)],
                focusDistance: fd, side: fd == 0 ? .focus : (id % 2 == 0 ? .upper : .lower),
                visibleAreaFraction: area, window: nil)
        }
        return [
            c(0, 0.2859, fd: 0), c(2, 0.1566, fd: 1), c(1, 0.1260, fd: 1),
    ...
    private func interior60(_ window: ClosedRange<Double>, tuning: GridTransitionTuning, phases: Int = 16) -> Int {
        ...
        for i in 0..<phases {
            ...
            let qs = GridTransitionScheduler.clickQSamples(
                durationSeconds: tuning.clickDurationSeconds,
    ...
    private func maxSimultaneousPartial(
        _ windows: [Int: ClosedRange<Double>], tuning: GridTransitionTuning,
        durationSeconds: Double, phases: Int = 50
    ) -> Int {
        let curve = tuning.localAlphaCurve
        var worst = 0
        for hz in [60.0, 120.0] {
            for i in 0..<phases {
                let ph = Double(i) / Double(phases)
                let qs = GridTransitionScheduler.clickQSamples(
                    durationSeconds: durationSeconds, hz: hz,
    ...
    @Test func alphaPeakSlopeBelowSmootherstep() {
        ...
        let n = 100_000
        for i in 0..<n {
            maxStep = max(maxStep, abs(f.value(Double(i + 1) / Double(n)) - f.value(Double(i) / Double(n))))
        }
        let peakSlope = maxStep * Double(n)
        #expect(peakSlope <= 1.26)  // ≈ 1.25, substantially below smootherstep's 1.875
    }

    @Test func allocationMeetsFrameBudgetTargets() {
        let comps = referenceComponents()
        // 420 ms: n=26, li=1, lo=3, so budget 22, so {focus:5, five:3, smallest:2}
        ...
        // 360 ms: budget 19, so {5,3,3,2,2,2,2}
        ...
        // 450 ms: budget 23, so {5,3,3,3,3,3,3} - every component ≥3 (2 interior)
    ...
    private func makeSyntheticPlan(window: ClosedRange<Double> = 0.3...0.7) -> GridTransitionPlan {
        ...
        let srcR: [RelativeSlotKey: CGRect] = [
            k0: unit, kA: unit.offsetBy(dx: 0, dy: 100), kB: unit.offsetBy(dx: 0, dy: -100),
        ]
        let tgtR: [RelativeSlotKey: CGRect] = [
            k0: unit, kA: unit.offsetBy(dx: 0, dy: 120), kB: unit.offsetBy(dx: 0, dy: -120),
        ]
        let lat = GridTransitionLattice(
            ...
            presentationSourceRect: srcR, presentationTargetRect: tgtR,  // all mixed, so presentation == real
            components: [
                .init(
                    id: 0, keys: [kA, kB], focusDistance: 1, side: .focus,
                    visibleAreaFraction: 0.2, window: nil)
            ])
        return GridTransitionComponentBuilder.assemble(
            kind: .click, lattice: lat, windows: [0: window],
    ...
    @Test func reverseEqualityExact() {
        let plan = makeSyntheticPlan()
        // lp is a pure function of q, so forward(q) == reverse(q). Compare lp at q vs at q traversed back.
        let curve = plan.curve
        for i in 0...1000 {
            let q = Double(i) / 1000
            let fwd = curve.localProgress(w0: 0.3, w1: 0.7, q: q)
            let rev = curve.localProgress(w0: 0.3, w1: 0.7, q: q)  // identical call, so deterministic, no hysteresis
            #expect(fwd == rev)
        }
    }
    ...
    private func makeBackgroundDissolvePlan(window: ClosedRange<Double> = 0.3...0.7) -> GridTransitionPlan {
        ...
        let lat = GridTransitionLattice(
            ...
            components: [
                .init(
                    id: 0, keys: [kS, kT], focusDistance: 1, side: .lower,
    ...
    @Test func mixedDissolveSourceOpaqueTargetAlpha() {
        ...
        // mixed key kA (src 200 to tgt 201): two draws, source then target
    ...
    @Test func sourceOnlyDissolveStaysTranslucent() {
        ...
        let draws = GridTransitionRendererInput.draws(plan: plan, at: 0.5)
        let lp = plan.curve.localProgress(w0: 0.3, w1: 0.7, q: 0.5)
        // id 300 at the source-only key fades to background, so alpha 1-lp (not opaque)
    ...
    @Test func targetOnlyDissolveAlphaLp() {
        ...
        let draws = GridTransitionRendererInput.draws(plan: plan, at: 0.5)
        let lp = plan.curve.localProgress(w0: 0.3, w1: 0.7, q: 0.5)
        let t = draws.first { $0.index == 300 && $0.isTarget }  // arrives from bg, so alpha lp
        #expect(t != nil)
    ...
    @Test func sourceOverWithOpaqueSourceEqualsLinearMix() {
        ...
        // Source remains opaque while target alpha advances, so the background cannot bleed through.
        let bg = 0.0
        let lp = 0.5
    ...
    @Test func mixedKeyStillInterpolatesRealSourceToRealTarget() {
        ...
        #expect(rectOf(kA, in: plan, at: 0) == src)  // q=0 to real source
        #expect(rectOf(kA, in: plan, at: 1) == tgt)  // q=1 to real target
    ...
    @Test func presentationTransformFitSynthesizeAndAbort() {
        ...
        var src: [RelativeSlotKey: CGRect] = [:]
        var tgt: [RelativeSlotKey: CGRect] = [:]
        ...
        #expect(tf != nil)
        #expect(
            abs(tf!.sx - 0.5) < 1e-6 && abs(tf!.sy - 0.5) < 1e-6 && abs(tf!.tx - 10) < 1e-6 && abs(tf!.ty - 20) < 1e-6)
        ...
        #expect(
            GridTransitionPresentationTransform.fit(keys: [common[0], entry, exit], sourceRect: src, targetRect: tgt)
                != nil)
        // no common rects, so abort (caller falls back to snap)
    ...
    @Test func selectionDoesNotGateTransitionGeometry() {
        ...
        #expect(GridTransitionSelectionEligibility.isEligible(selection: [], relocatingIdentities: relocating))
        #expect(GridTransitionSelectionEligibility.isEligible(selection: [100], relocatingIdentities: relocating))
        #expect(GridTransitionSelectionEligibility.isEligible(selection: [201], relocatingIdentities: relocating))
    ...
    @Test func builderOnEngineGeometryFocusRowStable() {
        ...
        // row 0 / col 0 at every level, so guaranteed common, relative key (0,0)).
        ...
        guard
            let lat = GridTransitionComponentBuilder.build(
                source: src, target: tgt, anchorIndex: anchor, viewportSize: viewport)
        else {
            Issue.record("lattice build returned nil")
            return
        }
        ...
        if let plan = ClickZoomTransitionScheduler.makePlan(
            source: src, target: tgt, anchorIndex: anchor, viewportSize: viewport)
        {
    ...
    @Test func pinchWindowsStructural() {
        ...
        #expect(windows.count == comps.count)
        for w in windows.values { #expect(abs((w.upperBound - w.lowerBound) - tuning.pinchWidthQ) < 1e-9) }
        let sorted = windows.values.sorted { $0.lowerBound < $1.lowerBound }
    ...
    @Test func pinchPlanEndpointsReproduceSourceAndTargetFrames() {
        ...
        guard
            let plan = PinchZoomTransitionScheduler.makePlan(
                source: src, target: tgt, anchorIndex: 0, viewportSize: viewport)
        else {
            Issue.record("pinch plan nil")
            return
        }
        #expect(plan.kind == .pinch)
        var srcRect: [Int: CGRect] = [:]
        for s in src.visibleSlots { srcRect[s.index] = s.viewportRect }
        var tgtRect: [Int: CGRect] = [:]
        for s in tgt.visibleSlots { tgtRect[s.index] = s.viewportRect }
        ...
        for d in atZero {  // q=0: every drawn occupant sits on its SOURCE slot
            if let r = srcRect[d.index] {
                #expect(approx(d.rect, r))
            } else {
                Issue.record("q=0 draw index \(d.index) absent from source frame")
            }
        }
        ...
        for d in atOne {  // q=1: every drawn occupant sits on its TARGET slot
            if let r = tgtRect[d.index] {
                #expect(approx(d.rect, r))
            } else {
                Issue.record("q=1 draw index \(d.index) absent from target frame")
            }
        }
    }
    ...
    // segment [3 to 2] at q=1 (settled L2) and segment [2 to 1] at q=0 (settled L2) draw the same occupants at the
    // same rects, so a continuous inter-segment seam (no blank frame, no snap) when the finger crosses a detent.
    @Test func chainingSeamSharedDetentIsContinuous() {
        ...
        guard
            let seg1 = PinchZoomTransitionScheduler.makePlan(
                source: f3, target: f2, anchorIndex: 0, viewportSize: viewport),
            let seg2 = PinchZoomTransitionScheduler.makePlan(
                source: f2, target: f1, anchorIndex: 0, viewportSize: viewport)
        else {
            Issue.record("segment plan nil")
            return
        }
    ...
    @Test func latticeEligibilityBoundary() {
        ...
        // Single-lattice scope = adjacent normal-level pairs (lo ∈ {0,1,2}, focusRowRelayout). The normal to
        // overview boundary (L3 to L4) is not eligible, so the live pinch falls back to the GridZoomTransaction reflow.
    ...
    @Test func chainBandIsContiguousFocusRowRelayoutRun() {
        ...
        #expect(band(around: 4).lo == band(around: 4).hi)  // overview start, so degenerate (reflow)
