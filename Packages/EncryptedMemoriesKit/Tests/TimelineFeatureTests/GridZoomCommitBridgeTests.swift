...
/// The release commit bridge: geometry-only morph from the `GridZoomTransaction` final frame to the settled
...
@Suite struct GridZoomCommitBridgeTests {
    ...
    private func setup() -> (e: SquareTileGridEngine, tx: GridZoomTransaction, phase: Int, scrollY: CGFloat) {
        ...
        let scrollY = e.anchoredScrollOffset(
            ...
            viewportPoint: tx.anchorViewportPoint, level: targetLevel, width: width, columnPhase: phase
        ).y
        return (e, tx, phase, scrollY)
    }

    private func bridge(
        _ e: SquareTileGridEngine, _ tx: GridZoomTransaction, _ scrollY: CGFloat, _ phase: Int, _ t: CGFloat
    ) -> [GridRenderSlot] {
        GridZoomCommitBridge.frame(
            transaction: tx, engine: e, targetLevel: targetLevel,
    ...
    @Test func commitBridgeMeasuresHorizontalPhaseShift() {
        let (e, tx, _, _) = setup()
        let d = e.commitDelta(transaction: tx, targetLevel: targetLevel, viewportSize: viewport)
    ...
    @Test func commitBridgeInterpolatesMatchedGlobalIndexRects() {
        ...
        let settled = e.framePlan(
            level: targetLevel, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: scrollY), overscan: overscan,
            columnPhase: phase)
        ...
        for t in stride(from: CGFloat(0), through: 1, by: 0.2) {
            ...
            #expect(
                abs(slot!.rect.minX - expectedX) < 0.01 && abs(slot!.rect.minY - expectedY) < 0.01,
    ...
    @Test func commitBridgeKeepsAnchorDeltaMonotonic() {
        ...
        for t in stride(from: CGFloat(0), through: 1, by: 0.1) {
            ...
            #expect(
                ...
                "anchor distance to settled not monotonic at t=\(t): \(distance) > \(lastDistance)")
    ...
    @Test func commitBridgeDoesNotChangeGlobalIdentity() {
        ...
        for t in stride(from: CGFloat(0), through: 1, by: 0.25) {
            ...
            #expect(
                ...
                "duplicate globalIndex in bridge frame at t=\(t) (would be duplicate chaos)")
            #expect(
                ids.sorted() == identitiesAt0.sorted(), "the set of visible global identities changed across t=\(t)")
        }
    }
    ...
    @Test func commitBridgeCompletesAtSettledGridFrame() {
        ...
        let settled = e.framePlan(
            level: targetLevel, viewportSize: viewport, scrollOffset: CGPoint(x: 0, y: scrollY), overscan: overscan,
            columnPhase: phase)
        let finalByIndex = Dictionary(
            uniqueKeysWithValues: bridge(e, tx, scrollY, phase, 1).map { ($0.index, $0.rect) })
        ...
        for s in settled.visibleSlots {
            ...
            #expect(
                abs(r!.minX - s.viewportRect.minX) < 0.01 && abs(r!.minY - s.viewportRect.minY) < 0.01
    ...
    @Test func commitBridgeDurationIsBounded() {
        #expect(
            GridZoomCommitBridge.duration >= 0.12 && GridZoomCommitBridge.duration <= 0.18,
