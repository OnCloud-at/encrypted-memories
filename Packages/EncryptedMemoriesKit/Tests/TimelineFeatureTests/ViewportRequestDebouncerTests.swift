...
/// Stable input emits the visible set once after the settle window.
...
struct ViewportRequestDebouncerTests {
    ...
    @Test func stationaryViewportEmitsExactlyOnce() {
        ...
        #expect(d.flushIfStable(at: 0.20) == set)  // settled, so emit once
    ...
    @Test func changingViewportNeverSettlesUntilItStops() {
        ...
        #expect(d.flushIfStable(at: 0.20) == uids(["c"]))  // now stable, so emit the final set once
    ...
    @Test func rearmDecisionEmitsFinalSetExactlyOnce() {
        ...
        // exactly once. Re-arming must key off the debouncer, not the caller's per-frame queue.
        let d = ViewportRequestDebouncer(window: 0.1)
        let a = uids(["a"])
        let b = uids(["b"])
        let c = uids(["c"])
        ...
        #expect(d.hasPendingUnflushed() == true)  // the caller re-arms another settle check
        #expect(d.flushIfStable(at: 0.15) == c)  // C has now been stable ≥ window, so emit once
        #expect(d.hasPendingUnflushed() == false)  // emitted, so no further re-arm
    ...
    @Test func newSetAfterFlushEmitsAgain() {
        ...
        #expect(d.flushIfStable(at: 0.45) == uids(["b"]))  // b settled, so emit
    }
}
