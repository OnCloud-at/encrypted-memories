...
/// in the bottom-right corner (column == cols-1; the last row is full, so no black area appears), and the only partial row is
/// the oldest at the top-left. The partial row remains at the top when column phase changes.
@Suite struct GridBottomRightAnchorTests {
        ...
        for count in [997, 2000, 5003, 12345, 20000] {
            ...
            for level in 0..<e.levelCount {
                ...
                guard let last = e.locate(flatIndex: count - 1, level: level, width: width) else {
                    Issue.record("no last item at level \(level)")
                    continue
                }
                // Newest in the bottom-right corner.
                #expect(
                    last.column == cols - 1,
                    "newest not bottom-right at level \(level), count \(count): col \(last.column)/\(cols)")
                ...
                if count > cols, let firstOfLastRow = e.locate(flatIndex: count - cols, level: level, width: width) {
                    #expect(
                        firstOfLastRow.row == last.row && firstOfLastRow.column == 0,
    ...
    @Test func noBlackOnTheRightOfTheBottomRowAtEveryLevel() {
        for count in [997, 5003, 20000] {
            ...
            for level in 0..<e.levelCount {
                ...
                #expect(
                    bottomRow.count == plan.columns,
                    "bottom row not full at level \(level), count \(count): \(bottomRow.count)/\(plan.columns)")
                let rightMost = bottomRow.map { $0.slotRect.maxX }.max()!
                #expect(rightMost <= width + 1.0, "bottom row overflows the width at level \(level)")
                #expect(
                    width - rightMost < 2.0,
                    "bottom row must FILL the width (right edge ≈ viewport, no gutter) at level \(level)")
            }
        }
    }
    ...
    @Test func topLeadingFillKeepsSparseRoutesInReadingOrder() {
        ...
        for level in 0..<e.levelCount {
            ...
            #expect(
                last.column == (count - 1) % cols, "bounded routes leave trailing empty cells, not leading empty cells")
            for flat in 0..<count {
                let loc = e.locate(flatIndex: flat, level: level, width: width)!
                #expect(
                    e.flatIndex(section: loc.section, row: loc.row, column: loc.column, level: level, width: width)
                        == flat)
            }
        }
    }
