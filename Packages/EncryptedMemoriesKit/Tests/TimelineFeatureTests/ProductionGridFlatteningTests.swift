...
/// Production MetalGrid flattens timeline sections into one ordered layout section.
/// Date and month labels still come from the source sections.
@MainActor
@Suite struct ProductionGridFlatteningTests {
    private func feed() -> ThumbnailFeed {
        ...
        return ThumbnailFeed(
            cache: ThumbnailCache(namespace: "test-\(UUID().uuidString)", rootDirectory: root),
    ...
    private func item(_ id: String, _ date: Date, video: Bool = false) -> PhotoItem {
        PhotoItem(
            uid: PhotoUID(volumeID: "v", nodeID: id), captureTime: date,
    ...
    @Test func productionDataSourceUsesSingleContinuousSection() {
        ...
        #expect(
            ds.sectionCounts == [16],
            "production must be ONE continuous section of all 16 items, got \(ds.sectionCounts)")
        #expect(ds.flatUIDs.count == 16)
        // Flat order preserves timeline order across the source sections.
        #expect(ds.flatUIDs.first == PhotoUID(volumeID: "v", nodeID: "jan-0"))
        #expect(ds.flatUIDs.last == PhotoUID(volumeID: "v", nodeID: "mar-3"))
        // An empty library produces no layout sections.
    ...
    @Test func productionMonthMarkersStillUseOriginalTimelineSections() {
        ...
        // items' capture dates, not from the single physical layout section.
        #expect(
            markers.map(\.index) == [0, 5, 12],
            "month markers must come from the items' dates, got \(markers.map(\.index))")
    ...
    @Test func productionDateMarkersSupportDayMonthYearGranularity() {
        ...
        let days = MetalGridProductionAdapter.dateMarkers(
            sections: sections, granularity: .day,
            calendar: Self.calendar, locale: Locale(identifier: "en_US_POSIX"))
        let months = MetalGridProductionAdapter.dateMarkers(
            sections: sections, granularity: .month,
            calendar: Self.calendar, locale: Locale(identifier: "en_US_POSIX"))
        let years = MetalGridProductionAdapter.dateMarkers(
            sections: sections, granularity: .year,
            calendar: Self.calendar, locale: Locale(identifier: "en_US_POSIX"))
    ...
    @Test func productionBeginZoomTransactionIsAvailable() {
        ...
        let tx = e.beginZoomTransaction(
            cursorContentPoint: CGPoint(x: 200, y: 300),
    ...
    @Test func productionGridTransactionNotDisabledByTimelineSections() {
        // Many date-grouped TimelineSections feed the data source, but it flattens them before passing them to the engine that the
        ...
        let manySections = (0..<40).map { s in
            TimelineSection(
                id: "2026-\(s)", date: day(2026, 1, 1), title: "S\(s)",
        ...
        let tx = engine.beginZoomTransaction(
            cursorContentPoint: CGPoint(x: 200, y: 500),
            viewportPoint: CGPoint(x: 200, y: 300), level: 3, width: 1400)
