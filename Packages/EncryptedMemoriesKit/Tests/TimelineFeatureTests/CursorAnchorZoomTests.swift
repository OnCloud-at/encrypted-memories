...
@Suite struct CursorAnchorZoomTests {
    ...
    private func cursorItemViewportY(
        ...
        cursorViewportY: CGFloat, toLevel: Int
    ) -> CGFloat? {
        ...
        guard let a = e.anchorItem(nearContentPoint: cursorContent, level: fromLevel, width: width),
            let ny = e.cursorAnchoredScrollOffsetY(
                levelChangeFrom: fromLevel, to: toLevel, width: width,
                cursorContentPoint: cursorContent, sourceScrollOriginY: scrollY),
            let frame = e.slotRect(flatIndex: a.flatIndex, level: toLevel, width: width)
        else { return nil }
    ...
    @Test func cursorAnchorItemRemainsUnderCursorAfterDiscreteZoom() {
        ...
        let cursorY: CGFloat = 600  // not the top of the viewport
        for toLevel in [0, 1, 3, 4, 5] {
            guard
                let y = cursorItemViewportY(
                    e, fromLevel: 2, scrollY: scrollY, cursorViewportY: cursorY, toLevel: toLevel)
            else {
                Issue.record("no anchor for level \(toLevel)")
                continue
            }
    ...
    @Test func pinchZoomUsesCursorAnchorNotViewportTop() {
        ...
        // The top item at viewport Y≈0 does not stay at the top under the cursor anchor.
    ...
    @Test func topViewportAnchorIsNotUsedForTrackpadPinch() {
        let e = engine()
        let scrollY: CGFloat = 6000
        let cursorOffset = e.cursorAnchoredScrollOffsetY(
            levelChangeFrom: 2, to: 4, width: width,
            cursorContentPoint: CGPoint(x: width / 2, y: scrollY + 600), sourceScrollOriginY: scrollY)!
        let topOffset = e.cursorAnchoredScrollOffsetY(
            levelChangeFrom: 2, to: 4, width: width,
    ...
    @Test func setLevelCanAnchorToExplicitItem() {
        ...
        for flat in [120, 777, 1500] {
            ...
            let off = e.anchoredScrollOffset(
                flatIndex: flat, localFraction: CGPoint(x: 0.5, y: 0.5), viewportPoint: vp, level: 3, width: width)
    ...
    @Test func gapCursorResolvesNearestFocusRowItem() {
        ...
        let plan = e.framePlan(
            level: level, viewportSize: CGSize(width: width, height: 900),
        ...
        guard let resolved = e.anchorItem(nearContentPoint: gapPoint, level: level, width: width) else {
            Issue.record("gap did not resolve to an item")
            return
        }
        #expect(resolved.flatIndex == a.index || resolved.flatIndex == b.index, "must resolve to an adjacent cell")
        #expect(
            abs(resolved.slotRect.midY - gapPoint.y) < e.resolvedMetrics(level: level, width: width).pitch,
            "resolved item must be in the focus row")
    }
}
