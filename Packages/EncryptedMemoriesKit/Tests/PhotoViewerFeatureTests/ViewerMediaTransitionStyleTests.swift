...
final class ViewerMediaTransitionStyleTests: XCTestCase {
    ...
    func testViewerReusesSharedTransitionStyleForMotionAndOriginalReveal() throws {
        ...
        XCTAssertFalse(
            view.contains(".easeInOut(duration: 0.18)"), "Live Photo fade duration must not be hardcoded in the view")
        XCTAssertFalse(
            view.contains(".easeInOut(duration: 0.3)"), "Live Photo scale duration must not be hardcoded in the view")
