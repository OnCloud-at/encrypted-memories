...
/// clamp to ±maxSpeed at or beyond the edges, allowing a drag-selection to run past the visible rows
/// without gaps.
@Suite struct GridEdgeAutoScrollPolicyTests {
    ...
    @Test func topBandScrollsUpAndRamps() {
        #expect(v(50) < 0)  // upper half of the band has negative velocity (toward top)
        #expect(v(0) == -maxSpeed)  // the very top has full negative speed
        #expect(v(-40) == -maxSpeed)  // dragging above the viewport clamps the speed
        // Monotonic: closer to the edge is faster (more negative).
        #expect(v(20) < v(80))
    }

    @Test func bottomBandScrollsDownAndRamps() {
        #expect(v(height - 50) > 0)  // lower half of the band has positive velocity (toward bottom)
        #expect(v(height) == maxSpeed)  // the very bottom has full positive speed
        #expect(v(height + 40) == maxSpeed)  // dragging below the viewport clamps the speed
        #expect(v(height - 20) > v(height - 80))
    }

    @Test func degenerateInputsYieldZero() {
        #expect(
            GridEdgeAutoScrollPolicy.velocity(touchY: 10, viewportHeight: 0, edgeInset: inset, maxSpeed: maxSpeed) == 0)
        #expect(
            GridEdgeAutoScrollPolicy.velocity(touchY: 10, viewportHeight: height, edgeInset: 0, maxSpeed: maxSpeed) == 0
        )
        #expect(
            GridEdgeAutoScrollPolicy.velocity(touchY: 10, viewportHeight: height, edgeInset: inset, maxSpeed: 0) == 0)
    }
    ...
    @Test func shortViewportKeepsDeadZone() {
        ...
        #expect(center == 0)
        #expect(
            GridEdgeAutoScrollPolicy.isInEdgeBand(touchY: short / 2, viewportHeight: short, edgeInset: bigInset)
                == false)
    }
