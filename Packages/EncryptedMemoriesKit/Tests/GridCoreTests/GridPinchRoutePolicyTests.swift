...
final class GridPinchRoutePolicyTests: XCTestCase {
    /// Production-shaped ladder: 0-3 are focus-row photo levels, 3 through 4 form the overview warp, and 4 to 5 the dense
    /// overview zoom.
    private let engine = SquareTileGridEngine(
        ...
        profile: GridLevelProfile(
            ...
            levels: [
                GridLevelMetrics(
                    levelID: 0, nominalColumns: 3, gap: 16, monthLabels: false, transitionKindToNext: .focusRowRelayout),
                GridLevelMetrics(
                    levelID: 1, nominalColumns: 5, gap: 12, monthLabels: false, transitionKindToNext: .focusRowRelayout),
                GridLevelMetrics(
                    levelID: 2, nominalColumns: 7, gap: 8, monthLabels: false, transitionKindToNext: .focusRowRelayout),
                GridLevelMetrics(
                    levelID: 3, nominalColumns: 9, gap: 6, monthLabels: false, transitionKindToNext: .overviewWarp),
                GridLevelMetrics(
                    levelID: 4, nominalColumns: 14, gap: 2, monthLabels: true, transitionKindToNext: .denseOverviewZoom),
    ...
    func testInBandStepRoutesToLattice() {
        ...
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 2, direction: -1, chainBand: band, engine: engine),
            .lattice(target: 1))
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 2, direction: 1, chainBand: band, engine: engine),
            .lattice(target: 3))
    ...
    func testOverviewBoundaryStepRoutesToDissolve() {
        ...
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 3, direction: 1, chainBand: band, engine: engine),
            .overviewDissolve(target: 4))
        let overviewBand = GridPinchRoutePolicy.chainBand(around: 4, engine: engine)
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 4, direction: 1, chainBand: overviewBand, engine: engine),
    ...
    func testLadderEdgesRouteToReflow() {
        ...
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 0, direction: -1, chainBand: band, engine: engine),
            .reflow)
        let topBand = GridPinchRoutePolicy.chainBand(around: 5, engine: engine)
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 5, direction: 1, chainBand: topBand, engine: engine),
            .reflow)
    }
    ...
    func testOutOfBandNonBoundaryStepRoutesToReflow() {
        ...
        XCTAssertEqual(
            GridPinchRoutePolicy.candidate(startLevel: 2, direction: -1, chainBand: (lo: 2, hi: 3), engine: engine),
            .reflow)
    }
}
