...
/// Delivers an original only after the test releases it. This models the SDK path that ignores task
/// cancellation while a cache clear advances the writer generation.
private actor BlockingOriginalProvider: FullMediaProvider {
    let bytes: Data
...
@Suite struct EncryptedOriginalProviderTests {
    ...
    private func makeCache() -> ThumbnailCache {
        // Default init uses a process-ephemeral key, so store to read round-trips within one instance work.
    ...
    /// Cache hits return cached bytes and do not call `FullMediaProvider.originalData`.
    ...
    /// With no cache configured, every request downloads and stores nothing.
    ...
    /// Read-only policy reuses a warm cache without growing it on a miss.
    ...
    @Test func destructiveClearRejectsLateOriginalWriter() async throws {
        let cache = makeCache()
        let provider = BlockingOriginalProvider(bytes: Data("late-original".utf8))
        let helper = EncryptedOriginalProvider(media: provider, cache: cache, policy: .persisting(capBytes: nil))
        let uid = uid("late-original")
        let load = Task { try await helper.originalData(for: uid) }

        for _ in 0..<40 {
            if await provider.hasEntered() { break }
            try await Task.sleep(for: .milliseconds(10))
        }
        #expect(await provider.hasEntered())
        await cache.clear()
    ...
    @Test func accountChangeRejectsLateOriginalOwner() async throws {
        let cache = makeCache()
        cache.configure(accountUID: "acct-A", key: SymmetricKey(size: .bits256))
        let provider = BlockingOriginalProvider(bytes: Data("account-a".utf8))
        let helper = EncryptedOriginalProvider(media: provider, cache: cache, policy: .persisting(capBytes: nil))
        let photo = uid("account-change")
        let load = Task { try await helper.originalData(for: photo) }
        ...
        #expect(await provider.hasEntered())
        cache.configure(accountUID: "acct-B", key: SymmetricKey(size: .bits256))
        await provider.release()

        do {
            _ = try await load.value
            Issue.record("An account-A owner must fail after account-B configuration")
        } catch let error as EncryptedOriginalProviderError {
            #expect(error == .sessionInvalidated)
        }
        #expect(cache.diskData(for: photo) == nil)
    }
}
