...
struct MediaDecodingCoreTests {
    @Test func downsampleProducesBoundedCGImageAndMetadata() throws {
        let decoded = try #require(
            ThumbnailImageDecoder.downsample(Self.pngData(width: 32, height: 16), maxPixelSize: 8))
...
struct MediaDecodingCorePlatformPurityTests {
    ...
    @Test func hasNoPlatformFrameworkImports() throws {
        ...
        #expect(
            violations.isEmpty,
            "MediaDecodingCore must not import platform UI frameworks:\n\(violations.joined(separator: "\n"))")
        #expect(
            seen.subtracting(Self.allowedFrameworkImports).isEmpty,
            "Unexpected MediaDecodingCore imports: \(seen.subtracting(Self.allowedFrameworkImports).sorted())")
    }
    ...
    @Test func hasNoPlatformImageTokens() throws {
        ...
        #expect(
            violations.isEmpty,
            "MediaDecodingCore must not reference platform UI image/view types:\n\(violations.joined(separator: "\n"))")
    }
