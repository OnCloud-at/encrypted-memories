...
final class RemotePhotoAssetProofBuilderTests: XCTestCase {
    ...
    func testBuildsSimpleAndCompoundProofsFromExactEncryptedMetadata() throws {
        ...
        XCTAssertEqual(
            records.first { $0.externalIdentity.identifier == "cloud-live" }?.remoteLinkIDs,
    ...
    func testAccumulatorRejectsAmbiguousIdentityAcrossPages() throws {
        let timestamp = "2026-07-10T07:00:00Z"
        let metadata = identity(cloudID: "same-cloud-id", timestamp: timestamp)
        var accumulator = RemotePhotoAssetProofBuilder.Accumulator(hashKeyEpoch: "epoch-1")

        accumulator.append(
            photos: [PhotosListEntry(linkID: "one", captureTime: 0, tags: [], relatedPhotos: [])],
            externalIdentitiesByLinkID: ["one": metadata]
        )
        accumulator.append(
            photos: [PhotosListEntry(linkID: "two", captureTime: 0, tags: [], relatedPhotos: [])],
            externalIdentitiesByLinkID: ["two": metadata]
        )
