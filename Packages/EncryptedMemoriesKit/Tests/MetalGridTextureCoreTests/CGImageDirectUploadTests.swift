...
@Suite struct CGImageDirectUploadTests {
    ...
    @Test func rgbxOpaqueDefault_forcesAlphaOne() {
        // memory R,G,B,X to (red, green, blue, 1)
        #expect(
            decide(alphaInfo: .noneSkipLast, byteOrder: .orderDefault)
    ...
    @Test func xrgbNoneSkipFirstDefault_theRealImageIOJPEGCase() {
        // memory X,R,G,B to sampled R=byte1(.green), G=byte2(.blue), B=byte3(.alpha), A=1
        #expect(
            decide(alphaInfo: .noneSkipFirst, byteOrder: .orderDefault)
    ...
    @Test func argbPremultipliedFirstDefault_pullsRealAlpha() {
        // memory A,R,G,B to R=byte1(.green), G=byte2(.blue), B=byte3(.alpha), A=byte0(.red)
        #expect(
            decide(alphaInfo: .premultipliedFirst, byteOrder: .orderDefault)
                == CGImageDirectUpload.Swizzle(red: .green, green: .blue, blue: .alpha, alpha: .red))
    }

    @Test func bgraPremultipliedFirstLittle_classicBGRA() {
        // memory B,G,R,A to R=byte2(.blue), G=byte1(.green), B=byte0(.red), A=byte3(.alpha)
        #expect(
            decide(alphaInfo: .premultipliedFirst, byteOrder: .order32Little)
                == CGImageDirectUpload.Swizzle(red: .blue, green: .green, blue: .red, alpha: .alpha))
    }

    @Test func bgrxNoneSkipFirstLittle_opaqueBGRX() {
        // memory B,G,R,X to R=.blue, G=.green, B=.red, A=1
        #expect(
            decide(alphaInfo: .noneSkipFirst, byteOrder: .order32Little)
                == CGImageDirectUpload.Swizzle(red: .blue, green: .green, blue: .red, alpha: .one))
    }

    @Test func rgbaPremultipliedLittle_reversedWord() {
        // logical R,G,B,A reversed to memory A,B,G,R to R=byte3(.alpha), G=byte2(.blue), B=byte1(.green), A=byte0(.red)
        #expect(
            decide(alphaInfo: .premultipliedLast, byteOrder: .order32Little)
    ...
    @Test func downsampleRequestFallsBack() throws {
        let image = try Self.solidImage(
            ...
            bitmap: CGImageAlphaInfo.noneSkipLast.rawValue)
        #expect(
            CGImageDirectUpload.swizzle(for: image, targetWidth: image.width / 2, targetHeight: image.height / 2) == nil
        )
    }
    ...
    @Test func swizzleReproducesRedrawOutputForDecodedJPEG() throws {
        ...
        let swizzle = try #require(
            CGImageDirectUpload.swizzle(for: thumb, targetWidth: thumb.width, targetHeight: thumb.height))

        // Path A: the current redraw into DeviceRGB premultipliedLast provides the reference pixels.
    ...
    private static func makeJPEG(
        width: Int, height: Int, colorSpace: CGColorSpace, r: CGFloat, g: CGFloat, b: CGFloat
    ) -> Data {
        let ctx = CGContext(
