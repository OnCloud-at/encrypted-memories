...
@Suite struct AppleSmartSearchRuntimeProviderTests {
    ...
    @Test func compiledModelDirectoryIsUsedDirectly() async throws {
        ...
        let resolved = try await AppleSmartSearchRuntimeProvider.loadableModelURL(
            artifactURL: compiled,
            runtimeCacheDirectory: dir.appendingPathComponent("runtime-cache")
        )
    ...
    @Test func installWithoutModelArtifactIsRejected() async throws {
        ...
        await #expect(throws: AppleSmartSearchRuntimeError.noModelArtifact) {
            _ = try await AppleSmartSearchRuntimeProvider.loadableModelURL(
                artifactURL: dir.appendingPathComponent("README.txt"),
                runtimeCacheDirectory: dir.appendingPathComponent("runtime-cache")
            )
        }
    }
