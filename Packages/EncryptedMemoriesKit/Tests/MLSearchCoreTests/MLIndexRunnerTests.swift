...
@Suite struct MLIndexRunnerTests {
    ...
    /// Embed calls are counted for recomputation checks.
    ...
    @Test func failureClassificationDrivesSchedulingAcrossPasses() async {
        ...
        #expect(first.progress.phase == .idle)  // transient pending is not complete
        #expect(!first.progress.isComplete)
    ...
    @Test func progressCallbackIsMonotonicAndHonest() async {
        ...
        let observer = MLIndexPassObserver(progressUpdated: { snapshots.append($0) })
        _ = await runner.runPass(
            allAssets: (0..<6).map { uid("a\($0)") },
            descriptor: descriptor,
            observer: observer
        )

        #expect(await waitUntil { snapshots.value.last?.phase == .completed })
    ...
    @Test func activityCallbackRequiresARealValidEmbedding() async {
        let store = InMemoryMLIndexStore()
        let permanent = uid("permanent")
        let transient = uid("transient")
        let embedder = ScriptedEmbedder(scripts: [permanent: .permanent, transient: .transientOnce])
        let runner = MLIndexRunner(store: store, embedder: embedder)
        let activity = ProgressCollector()
        let observer = MLIndexPassObserver(embeddingProduced: { activity.append($0) })

        _ = await runner.runPass(
            allAssets: [permanent, transient],
            descriptor: descriptor,
            observer: observer
        )
        try? await Task.sleep(for: .milliseconds(50))
        #expect(activity.value.isEmpty, "permanent failures and thumbnail/cache misses are not indexing activity")
        ...
        #expect(await waitUntil { activity.value.count == 1 })
    }

    @Test func semanticQuantumPersistsBoundedWorkAndResumesWithoutReembedding() async {
        let store = InMemoryMLIndexStore()
        let embedder = ScriptedEmbedder()
        let runner = MLIndexRunner(
            store: store,
            embedder: embedder,
            configuration: .init(chunkSize: 8)
        )
        let assets = (0..<20).map { uid("a\($0)") }
        ...
        #expect(!first.ranToCompletion)
        #expect(first.report.indexed == 5)
        #expect(store.count(for: descriptor) == 5)

        let second = await runner.runPass(
            allAssets: assets,
            descriptor: descriptor,
            maximumAssets: 5,
            libraryGeneration: 1
        ...
        #expect(!second.ranToCompletion)
        #expect(second.report.indexed == 5)
        #expect(store.count(for: descriptor) == 10)
        #expect(embedder.totalCalls == 10)
    }

    @Test func semanticLeaseIsCheckedBeforeEachAssetAndResumeDoesNotReembed() async {
        let store = InMemoryMLIndexStore()
        let embedder = ScriptedEmbedder()
        let runner = MLIndexRunner(
            store: store,
            embedder: embedder,
            configuration: .init(chunkSize: 16)
        )
        let assets = (0..<12).map { uid("lease-\($0)") }
        let lease = LockedGate(remainingAssets: 3)

        let interrupted = await runner.runPass(
            allAssets: assets,
            descriptor: descriptor,
            maximumAssets: 12,
            libraryGeneration: 1,
            passShouldContinue: { lease.consumePermission() }
        )
        #expect(!interrupted.ranToCompletion)
        #expect(interrupted.report.indexed == 3)
        #expect(embedder.totalCalls == 3)

        let resumed = await runner.runPass(
            allAssets: assets,
            descriptor: descriptor,
            maximumAssets: 12,
            libraryGeneration: 1
        )
        #expect(resumed.ranToCompletion)
        #expect(resumed.report.indexed == 9)
        #expect(store.count(for: descriptor) == assets.count)
        #expect(embedder.totalCalls == assets.count)
    }

    @Test func semanticQuantumDefersTransientAssetWithoutBlockingFreshWork() async {
        let store = InMemoryMLIndexStore()
        let flaky = uid("flaky")
        let embedder = ScriptedEmbedder(scripts: [flaky: .transientOnce])
        let runner = MLIndexRunner(
            store: store,
            embedder: embedder,
            configuration: .init(chunkSize: 4)
        )
        let assets = [flaky] + (0..<7).map { uid("fresh\($0)") }
        ...
        #expect(!second.ranToCompletion)
        #expect(second.report.indexed == 4)
        #expect(embedder.callCount(for: flaky) == 1)

        let third = await runner.runPass(
            allAssets: assets,
            descriptor: descriptor,
            maximumAssets: 4,
            libraryGeneration: 1
        )
        #expect(third.ranToCompletion)
        #expect(third.report.indexed == 1)
        #expect(embedder.callCount(for: flaky) == 2)
        #expect(store.count(for: descriptor) == assets.count)
    }
}
