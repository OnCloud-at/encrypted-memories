...
final class PhotoLocationFramingTests: XCTestCase {
    private func coord(_ lat: Double, _ lon: Double, _ n: Int = 0) -> PhotoCoordinate {
        PhotoCoordinate(
            uid: PhotoUID(volumeID: "v", nodeID: "n\(n)"), latitude: lat, longitude: lon,
    ...
    func testCompactClusterIsFullyContained() throws {
        ...
        for c in coords {
            XCTAssertTrue(
                box.contains(latitude: c.latitude, longitude: c.longitude),
                "dropped a non-outlier at \(c.latitude),\(c.longitude)")
        }
    }
}
