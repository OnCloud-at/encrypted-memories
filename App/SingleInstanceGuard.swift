...
final class SingleInstanceGuard {
    ...
    func acquire() -> Bool {
        ...
        do {
            ...
            NSLog("EncryptedMemories single-instance lock directory failed: \(error)")
            return false
        }

        let descriptor = open(lockURL.path, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR)
        guard descriptor >= 0 else {
            NSLog("EncryptedMemories single-instance lock open failed: errno=\(errno)")
            return false
        }
        ...
        if lockErrno == EWOULDBLOCK || lockErrno == EAGAIN {
            NSLog("EncryptedMemories duplicate launch ignored")
        } else {
            NSLog("EncryptedMemories single-instance lock failed: errno=\(lockErrno)")
        }
        return false
    }
    ...
    private static func defaultLockURL() -> URL {
        ...
        let namespace = Bundle.main.bundleIdentifier ?? "at.oncloud.encryptedmemories"
        return
            root
